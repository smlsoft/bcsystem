...
class SegmentedButton<T> extends StatefulWidget {
  ...
  static ButtonStyle styleFrom({
    ...
    final MaterialStateProperty<Color?>? foregroundColorProp = (foregroundColor == null && disabledForegroundColor == null && selectedForegroundColor == null)
        ? null
        : _SegmentButtonDefaultColor(foregroundColor, disabledForegroundColor, selectedForegroundColor);
    final MaterialStateProperty<Color?>? backgroundColorProp = (backgroundColor == null && disabledBackgroundColor == null && selectedBackgroundColor == null)
        ? null
        : _SegmentButtonDefaultColor(backgroundColor, disabledBackgroundColor, selectedBackgroundColor);
    final MaterialStateProperty<Color?>? overlayColor = (foregroundColor == null) ? null : _SegmentedButtonDefaultOverlay(foregroundColor);
    final MaterialStateProperty<MouseCursor?>? mouseCursor =
        (enabledMouseCursor == null && disabledMouseCursor == null) ? null : _SegmentedButtonDefaultMouseCursor(enabledMouseCursor, disabledMouseCursor);
    return ButtonStyle(
...
class SegmentedButtonState<T> extends State<SegmentedButton<T>> {
  ...
  void _handleOnPressed(T segmentValue) {
    ...
    if (validChange) {
      ...
      if (toggle) {
        updatedSelection = widget.selected.contains(segmentValue) ? widget.selected.difference(pressedSegment) : widget.selected.union(pressedSegment);
      } else {
  ...
  Widget build(BuildContext context) {
    ...
    final Widget? selectedIcon = widget.showSelectedIcon ? widget.selectedIcon ?? theme.selectedIcon ?? defaults.selectedIcon : null;
...
class _RenderSegmentedButton<T> extends RenderBox
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    while (child != null) {
      ...
      final Rect segmentClipRect = Rect.fromLTRB(segmentLeft, borderRect.top - borderOutset, segmentRight, borderRect.bottom + borderOutset);
      ...
      if (previousChild != null) {
        final BorderSide divider =
            segments[index - 1].enabled || segments[index].enabled ? enabledBorder.side.copyWith(strokeAlign: 0.0) : disabledBorder.side.copyWith(strokeAlign: 0.0);

...
class MasterPatternScreen extends StatefulWidget {
  const MasterPatternScreen({Key? key}) : super(key: key);

  @override
  State<MasterPatternScreen> createState() => MasterPatternScreenState();
}

class MasterPatternScreenState extends State<MasterPatternScreen> with SingleTickerProviderStateMixin {
  ...
  List<String> patternGuidListChecked = [];
  ...
  late MasterPatternState blocMasterPatternState;
  String headerEdit = "";
  ...
  void loadDataList(String search) {
    context.read<MasterPatternBloc>().add(MasterPatternLoadList(offset: (listData.isEmpty) ? 0 : listData.length, limit: global.loadDataPerPage, search: search));
  }
  ...
  void getData(String guid) {
    ...
    context.read<MasterPatternBloc>().add(MasterPatternGet(guid: guid));
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: <Widget>[
          ...
          if (patternGuidListChecked.isNotEmpty)
            Padding(
                padding: const EdgeInsets.only(right: 20.0),
                child: IconButton(
                  focusNode: FocusNode(skipTraversal: true),
                      ...
                      builder: (BuildContext context) => AlertDialog(
                        ...
                        actions: <Widget>[
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () {
                                Navigator.pop(context);
                                context.read<MasterPatternBloc>().add(MasterPatternDeleteMany(guid: patternGuidListChecked));
                              },
        ...
        children: [
          ...
          Container(
              ...
              child: Row(children: [
                Expanded(
                    flex: 5,
                    child: Text(global.language("pattern_code"),
          ...
          Expanded(
              child: RawKeyboardListener(
                  ...
                  onKey: (RawKeyEvent event) {
                    // keyboard handling similar to MasterBrandScreen
                  },
  ...
  void switchToEdit(MasterPatternModel value) {
    setState(() {
  ...
  Widget listObject(int index, MasterPatternModel value, bool showCheckBox) {
    bool isCheck = false;
    for (int i = 0; i < patternGuidListChecked.length; i++) {
      if (patternGuidListChecked[i] == value.guidfixed) {
        isCheck = true;
        break;
      }
    }
        ...
        onTap: () {
          if (showCheckBox == true) {
            setState(() {
              selectGuid = value.guidfixed;
              if (isCheck == true) {
                patternGuidListChecked.remove(value.guidfixed);
              } else {
                patternGuidListChecked.add(value.guidfixed);
              }
              global.showSnackBar(
                  context,
                  const Icon(
                  ...
                  "${global.language("chosen")} ${patternGuidListChecked.length} ${global.language("list")}",
                  Colors.blue);
            });
          } else {
            setState(() {
  ...
  bool verifyData(MasterPatternModel value) {
  ...
  void saveOrUpdateData() {
    if (screenEvent == global.ScreenEventEnum.edit || screenEvent == global.ScreenEventEnum.add) {
      MasterPatternModel pattern = MasterPatternModel(
        guidfixed: "",
      ...
      if (verifyData(pattern)) {
        ...
        if (selectGuid.trim().isEmpty) {
          context.read<MasterPatternBloc>().add(MasterPatternSave(patternModel: pattern));
        } else {
          context.read<MasterPatternBloc>().add(MasterPatternUpdate(guid: selectGuid, patternModel: pattern));
        }
      }
    }
  }

  void getDataToEditScreen(MasterPatternModel pattern) {
    isChange = false;
    selectGuid = pattern.guidfixed;
    fieldTextController[0].text = pattern.code;
    for (int i = 0; i < languageList.length; i++) {
      fieldTextController[i + 1].text = "";
    }
    for (int i = 0; i < languageList.length; i++) {
      for (int j = 0; j < pattern.names.length; j++) {
        if (languageList[i].code! == pattern.names[j].code) {
          fieldTextController[i + 1].text = pattern.names[j].name;
        }
      }
    }
  }
    ...
    return Scaffold(
        ...
        appBar: AppBar(
            ...
            leading: mobileScreen
                ...
                : null,
            title: Text(headerEdit + global.language("pattern")),
            actions: <Widget>[
                          ...
                          builder: (BuildContext context) => AlertDialog(
                            ...
                            actions: <Widget>[
                              ...
                              ElevatedButton(
                                  ...
                                  onPressed: () {
                                    Navigator.pop(context);
                                    context.read<MasterPatternBloc>().add(MasterPatternDelete(guid: selectGuid));
                                  },
                                  child: Text(global.language('confirm'))),
                            ],
                          ),
                      ...
                      TextFormField(
                         inputFormatters: <TextInputFormatter>[
                          ...
                          decoration: InputDecoration(
                            ...
                            labelText: global.language("pattern_code"),
                        ...
                        Padding(
                          ...
                          child: TextFormField(
                            ...
                            decoration: InputDecoration(
                              ...
                              border: const OutlineInputBorder(),
                              labelText: "${global.language("pattern_name")} (${languageList[i].name})",
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: LayoutBuilder(builder: (context, constraints) {
          return BlocListener<MasterPatternBloc, MasterPatternState>(
              listener: (context, state) {
                blocMasterPatternState = state;
                if (state is MasterPatternLoadSuccess) {
                  setState(() {
                    if (state.patterns.isNotEmpty) {
                      listData.addAll(state.patterns);
                    }
                  });
                }
                if (state is MasterPatternSaveSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterPatternSaveFailed) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterPatternUpdateSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterPatternUpdateFailed) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterPatternDeleteSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterPatternDeleteManySuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterPatternGetSuccess) {
                  setState(() {
                    getDataToEditScreen(state.pattern);
                    if (screenEvent == global.ScreenEventEnum.edit) {
                      WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
                        tabController.animateTo(1);
                      });

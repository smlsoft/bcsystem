...
class MasterGroupScreen extends StatefulWidget {
  const MasterGroupScreen({Key? key}) : super(key: key);

  @override
  State<MasterGroupScreen> createState() => MasterGroupScreenState();
}

class MasterGroupScreenState extends State<MasterGroupScreen> with SingleTickerProviderStateMixin {
  ...
  List<String> groupGuidListChecked = [];
  ...
  late MasterGroupState blocMasterGroupState;
  String headerEdit = "";
  ...
  void loadDataList(String search) {
    context.read<MasterGroupBloc>().add(MasterGroupLoadList(offset: (listData.isEmpty) ? 0 : listData.length, limit: global.loadDataPerPage, search: search));
  }
  ...
  void getData(String guid) {
    ...
    context.read<MasterGroupBloc>().add(MasterGroupGet(guid: guid));
  }
  ...
  Widget listScreen({bool mobileScreen = false}) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(global.language('group')),
        leading: IconButton(
        ...
        actions: <Widget>[
          ...
          if (groupGuidListChecked.isNotEmpty)
            Padding(
                padding: const EdgeInsets.only(right: 20.0),
                child: IconButton(
                  focusNode: FocusNode(skipTraversal: true),
                      ...
                      builder: (BuildContext context) => AlertDialog(
                        ...
                        actions: <Widget>[
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () {
                                Navigator.pop(context);
                                context.read<MasterGroupBloc>().add(MasterGroupDeleteMany(guid: groupGuidListChecked));
                              },
        ...
        children: [
          ...
          Container(
              ...
              child: Row(children: [
                Expanded(
                    flex: 5,
                    child: Text(global.language("group_code"),
          ...
          Expanded(
              child: RawKeyboardListener(
                  ...
                  onKey: (RawKeyEvent event) {
                    // Keyboard navigation handling similar to MasterBrandScreen
                  },
  ...
  void switchToEdit(MasterGroupModel value) {
    setState(() {
  ...
  Widget listObject(int index, MasterGroupModel value, bool showCheckBox) {
    bool isCheck = false;
    for (int i = 0; i < groupGuidListChecked.length; i++) {
      if (groupGuidListChecked[i] == value.guidfixed) {
        isCheck = true;
        break;
      }
    }
        ...
        onTap: () {
          if (showCheckBox == true) {
            setState(() {
              selectGuid = value.guidfixed;
              if (isCheck == true) {
                groupGuidListChecked.remove(value.guidfixed);
              } else {
                groupGuidListChecked.add(value.guidfixed);
              }
              global.showSnackBar(
                  context,
                  const Icon(
                  ...
                  "${global.language("chosen")} ${groupGuidListChecked.length} ${global.language("list")}",
                  Colors.blue);
            });
          } else {
            setState(() {
  ...
  bool verifyData(MasterGroupModel value) {
  ...
  void saveOrUpdateData() {
    if (screenEvent == global.ScreenEventEnum.edit || screenEvent == global.ScreenEventEnum.add) {
      MasterGroupModel group = MasterGroupModel(
        guidfixed: "",
      ...
      if (verifyData(group)) {
        ...
        if (selectGuid.trim().isEmpty) {
          context.read<MasterGroupBloc>().add(MasterGroupSave(groupModel: group));
        } else {
          context.read<MasterGroupBloc>().add(MasterGroupUpdate(guid: selectGuid, groupModel: group));
        }
      }
    }
  }

  void getDataToEditScreen(MasterGroupModel group) {
    isChange = false;
    selectGuid = group.guidfixed;
    fieldTextController[0].text = group.code;
    for (int i = 0; i < languageList.length; i++) {
      fieldTextController[i + 1].text = "";
    }
    for (int i = 0; i < languageList.length; i++) {
      for (int j = 0; j < group.names.length; j++) {
        if (languageList[i].code! == group.names[j].code) {
          fieldTextController[i + 1].text = group.names[j].name;
        }
      }
    }
  }
    ...
    return Scaffold(
        ...
        appBar: AppBar(
            ...
            leading: mobileScreen
                ...
                : null,
            title: Text(headerEdit + global.language("group")),
            actions: <Widget>[
                          ...
                          builder: (BuildContext context) => AlertDialog(
                            ...
                            actions: <Widget>[
                              ...
                              ElevatedButton(
                                  ...
                                  onPressed: () {
                                    Navigator.pop(context);
                                    context.read<MasterGroupBloc>().add(MasterGroupDelete(guid: selectGuid));
                                  },
                                  child: Text(global.language('confirm'))),
                            ],
                          ),
                      ...
                      TextFormField(
                         inputFormatters: <TextInputFormatter>[
                          ...
                          decoration: InputDecoration(
                            ...
                            labelText: global.language("group_code"),
                        ...
                        Padding(
                          ...
                          child: TextFormField(
                            ...
                            decoration: InputDecoration(
                              ...
                              border: const OutlineInputBorder(),
                              labelText: "${global.language("group_name")} (${languageList[i].name})",
        ...
        body: LayoutBuilder(builder: (context, constraints) {
          return BlocListener<MasterGroupBloc, MasterGroupState>(
              listener: (context, state) {
                blocMasterGroupState = state;
                if (state is MasterGroupLoadSuccess) {
                  setState(() {
                    if (state.groups.isNotEmpty) {
                      listData.addAll(state.groups);
                    }
                  });
                }
                if (state is MasterGroupSaveSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterGroupSaveFailed) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterGroupUpdateSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterGroupUpdateFailed) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterGroupDeleteSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterGroupDeleteManySuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterGroupGetSuccess) {
                  setState(() {
                    getDataToEditScreen(state.group);
                    if (screenEvent == global.ScreenEventEnum.edit) {
                      WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
                        tabController.animateTo(1);
                      });

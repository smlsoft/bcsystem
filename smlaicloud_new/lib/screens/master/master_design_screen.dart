...
class MasterDesignScreen extends StatefulWidget {
  const MasterDesignScreen({Key? key}) : super(key: key);

  @override
  State<MasterDesignScreen> createState() => MasterDesignScreenState();
}

class MasterDesignScreenState extends State<MasterDesignScreen>
  ...
  List<String> designGuidListChecked = [];
  ...
  late MasterDesignState blocMasterDesignState;
  String headerEdit = "";
  ...
  void getData(String guid) {
    ...
    context.read<MasterDesignBloc>().add(MasterDesignGet(guid: guid));
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: <Widget>[
          ...
          if (designGuidListChecked.isNotEmpty)
            Padding(
                padding: const EdgeInsets.only(right: 20.0),
                child: IconButton(
                  focusNode: FocusNode(skipTraversal: true),
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () {
                                ...
                                context.read<MasterDesignBloc>().add(
                                    MasterDesignDeleteMany(
                                        guid: designGuidListChecked));
                              },
        ...
        children: [
          ...
          Container(
              ...
              child: Row(children: [
                Expanded(
                    flex: 5,
                    child: Text(global.language("design_code"),
                        style: TextStyle(
  ...
  void switchToEdit(MasterDesignModel value) {
    setState(() {
  ...
  Widget listObject(int index, MasterDesignModel value, bool showCheckBox) {
    bool isCheck = false;
    for (int i = 0; i < designGuidListChecked.length; i++) {
      if (designGuidListChecked[i] == value.guidfixed) {
        isCheck = true;
        break;
      }
    }
        ...
        onTap: () {
          if (showCheckBox == true) {
            setState(() {
              selectGuid = value.guidfixed;
              if (isCheck == true) {
                designGuidListChecked.remove(value.guidfixed);
              } else {
                designGuidListChecked.add(value.guidfixed);
              }
              global.showSnackBar(
                  context,
                  const Icon(
                  ...
                  "${global.language("chosen")} ${designGuidListChecked.length} ${global.language("list")}",
                  Colors.blue);
            });
          } else {
            setState(() {
  ...
  bool verifyData(MasterDesignModel value) {
  ...
  void saveOrUpdateData() {
    if (screenEvent == global.ScreenEventEnum.edit ||
      ...
      MasterDesignModel design = MasterDesignModel(
        guidfixed: "",
        code: fieldTextController[0].text,
        names: packLanguage(),
      );
      if (verifyData(design)) {
        showCheckBox = false;
        if (selectGuid.trim().isEmpty) {
          context
              .read<MasterDesignBloc>()
              .add(MasterDesignSave(designModel: design));
        } else {
          context
              .read<MasterDesignBloc>()
              .add(MasterDesignUpdate(guid: selectGuid, designModel: design));
        }
      }
    }
  }

  void getDataToEditScreen(MasterDesignModel design) {
    isChange = false;
    selectGuid = design.guidfixed;
    fieldTextController[0].text = design.code;
    for (int i = 0; i < languageList.length; i++) {
      fieldTextController[i + 1].text = "";
    }
    for (int i = 0; i < languageList.length; i++) {
      for (int j = 0; j < design.names.length; j++) {
        if (languageList[i].code! == design.names[j].code) {
          fieldTextController[i + 1].text = design.names[j].name;
        }
      }
    }
  }
                            ...
                            actions: <Widget>[
                              ...
                              ElevatedButton(
                                  ...
                                  onPressed: () {
                                    Navigator.pop(context);
                                    context.read<MasterDesignBloc>().add(
                                        MasterDesignDelete(guid: selectGuid));
                                  },
                                  child: Text(global.language('confirm'))),
                            ],
                          ),
                    ...
                    child: Column(children: [
                      ...
                      TextFormField(
                          ...
                          decoration: InputDecoration(
                            ...
                            labelText: global.language("design_code"),
                        ...
                        Padding(
                          ...
                          child: TextFormField(
                            ...
                            decoration: InputDecoration(
                              ...
                              border: const OutlineInputBorder(),
                              labelText:
                                  "${global.language("design_name")} (${languageList[i].name})",
        ...
        body: LayoutBuilder(builder: (context, constraints) {
          return BlocListener<MasterDesignBloc, MasterDesignState>(
              listener: (context, state) {
                blocMasterDesignState = state;
                if (state is MasterDesignLoadSuccess) {
                  setState(() {
                    if (state.designs.isNotEmpty) {
                      listData.addAll(state.designs);
                    }
                  });
                }
                if (state is MasterDesignSaveSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterDesignSaveFailed) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterDesignUpdateSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterDesignUpdateFailed) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterDesignDeleteSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterDesignDeleteManySuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterDesignGetSuccess) {
                  setState(() {
                    getDataToEditScreen(state.design);
                    if (screenEvent == global.ScreenEventEnum.edit) {
                      WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
                        tabController.animateTo(1);
                      });

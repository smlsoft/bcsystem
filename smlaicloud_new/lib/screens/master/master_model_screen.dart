...
class MasterModelScreen extends StatefulWidget {
  const MasterModelScreen({Key? key}) : super(key: key);

  @override
  State<MasterModelScreen> createState() => MasterModelScreenState();
}

class MasterModelScreenState extends State<MasterModelScreen> with SingleTickerProviderStateMixin {
  ...
  List<String> modelGuidListChecked = [];
  ...
  late MasterModelState blocMasterModelState;
  String headerEdit = "";
  ...
  void loadDataList(String search) {
    context.read<MasterModelBloc>().add(MasterModelLoadList(offset: (listData.isEmpty) ? 0 : listData.length, limit: global.loadDataPerPage, search: search));
  }
  ...
  void getData(String guid) {
    ...
    context.read<MasterModelBloc>().add(MasterModelGet(guid: guid));
  }
  ...
  Widget listScreen({bool mobileScreen = false}) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(global.language('model')),
        leading: IconButton(
        ...
        actions: <Widget>[
          ...
          if (modelGuidListChecked.isNotEmpty)
            Padding(
                padding: const EdgeInsets.only(right: 20.0),
                child: IconButton(
                  focusNode: FocusNode(skipTraversal: true),
                      ...
                      builder: (BuildContext context) => AlertDialog(
                        ...
                        actions: <Widget>[
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () {
                                Navigator.pop(context);
                                context.read<MasterModelBloc>().add(MasterModelDeleteMany(guid: modelGuidListChecked));
                              },
        ...
        children: [
          ...
          Container(
              ...
              child: Row(children: [
                Expanded(
                    flex: 5,
                    child: Text(global.language("model_code"),
  ...
  void switchToEdit(MasterModelModel value) {
    setState(() {
  ...
  Widget listObject(int index, MasterModelModel value, bool showCheckBox) {
    bool isCheck = false;
    for (int i = 0; i < modelGuidListChecked.length; i++) {
      if (modelGuidListChecked[i] == value.guidfixed) {
        isCheck = true;
        break;
      }
    }
        ...
        onTap: () {
          if (showCheckBox == true) {
            setState(() {
              selectGuid = value.guidfixed;
              if (isCheck == true) {
                modelGuidListChecked.remove(value.guidfixed);
              } else {
                modelGuidListChecked.add(value.guidfixed);
              }
              global.showSnackBar(
                  context,
                  const Icon(
                  ...
                  "${global.language("chosen")} ${modelGuidListChecked.length} ${global.language("list")}",
                  Colors.blue);
            });
          } else {
            setState(() {
  ...
  bool verifyData(MasterModelModel value) {
  ...
  void saveOrUpdateData() {
    if (screenEvent == global.ScreenEventEnum.edit || screenEvent == global.ScreenEventEnum.add) {
      MasterModelModel model = MasterModelModel(
        guidfixed: "",
      ...
      if (verifyData(model)) {
        ...
        if (selectGuid.trim().isEmpty) {
          context.read<MasterModelBloc>().add(MasterModelSave(modelModel: model));
        } else {
          context.read<MasterModelBloc>().add(MasterModelUpdate(guid: selectGuid, modelModel: model));
        }
      }
    }
  }

  void getDataToEditScreen(MasterModelModel model) {
    isChange = false;
    selectGuid = model.guidfixed;
    fieldTextController[0].text = model.code;
    for (int i = 0; i < languageList.length; i++) {
      fieldTextController[i + 1].text = "";
    }
    for (int i = 0; i < languageList.length; i++) {
      for (int j = 0; j < model.names.length; j++) {
        if (languageList[i].code! == model.names[j].code) {
          fieldTextController[i + 1].text = model.names[j].name;
        }
      }
    }
  }
    ...
    return Scaffold(
        ...
        appBar: AppBar(
            ...
            leading: mobileScreen
                ...
                : null,
            title: Text(headerEdit + global.language("model")),
            actions: <Widget>[
                          ...
                          builder: (BuildContext context) => AlertDialog(
                            ...
                            actions: <Widget>[
                              ...
                              ElevatedButton(
                                  ...
                                  onPressed: () {
                                    Navigator.pop(context);
                                    context.read<MasterModelBloc>().add(MasterModelDelete(guid: selectGuid));
                                  },
                                  child: Text(global.language('confirm'))),
                            ],
                          ),
                      ...
                      TextFormField(
                         inputFormatters: <TextInputFormatter>[
                          ...
                          decoration: InputDecoration(
                            ...
                            labelText: global.language("model_code"),
                        ...
                        Padding(
                          ...
                          child: TextFormField(
                            ...
                            decoration: InputDecoration(
                              ...
                              border: const OutlineInputBorder(),
                              labelText: "${global.language("model_name")} (${languageList[i].name})",
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: LayoutBuilder(builder: (context, constraints) {
          return BlocListener<MasterModelBloc, MasterModelState>(
              listener: (context, state) {
                blocMasterModelState = state;
                if (state is MasterModelLoadSuccess) {
                  setState(() {
                    if (state.models.isNotEmpty) {
                      listData.addAll(state.models);
                    }
                  });
                }
                if (state is MasterModelSaveSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterModelSaveFailed) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterModelUpdateSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterModelUpdateFailed) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterModelDeleteSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterModelDeleteManySuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterModelGetSuccess) {
                  setState(() {
                    getDataToEditScreen(state.model);
                    if (screenEvent == global.ScreenEventEnum.edit) {
                      WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
                        tabController.animateTo(1);
                      });

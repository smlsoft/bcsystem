...
class MasterClassScreen extends StatefulWidget {
  ...
  @override
  State<MasterClassScreen> createState() => MasterClassScreenState();
}

class MasterClassScreenState extends State<MasterClassScreen>
  ...
  List<String> classGuidListChecked = [];
  ...
  late MasterClassState blocMasterClassState;
  String headerEdit = "";
  ...
  void getData(String guid) {
    ...
    context.read<MasterClassBloc>().add(MasterClassGet(guid: guid));
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: <Widget>[
          ...
          if (classGuidListChecked.isNotEmpty)
            Padding(
                padding: const EdgeInsets.only(right: 20.0),
                child: IconButton(
                  focusNode: FocusNode(skipTraversal: true),
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () {
                                ...
                                context.read<MasterClassBloc>().add(
                                    MasterClassDeleteMany(
                                        guid: classGuidListChecked));
                              },
        ...
        children: [
          ...
          Container(
              ...
              child: Row(children: [
                Expanded(
                    flex: 5,
                    child: Text(global.language("class_code"),
                        style: TextStyle(
                    ...
                    if (event is RawKeyDownEvent) {
                      if (event.logicalKey == LogicalKeyboardKey.arrowUp) {
                        ...
                        if (currentListIndex >= 0 &&
                          ...
                          MasterClassModel value = listData[currentListIndex];
                          if (showCheckBox == true) {
                            setState(() {
                              selectGuid = value.guidfixed;
                              if (classGuidListChecked
                                  .contains(value.guidfixed)) {
                                classGuidListChecked.remove(value.guidfixed);
                              } else {
                                classGuidListChecked.add(value.guidfixed);
                              }
                            });
                          } else {
                            discardData(callBack: () {
  ...
  void switchToEdit(MasterClassModel value) {
    setState(() {
      selectGuid = value.guidfixed;
      getData(selectGuid);
  ...
  Widget listObject(int index, MasterClassModel value, bool showCheckBox) {
    bool isCheck = false;
    for (int i = 0; i < classGuidListChecked.length; i++) {
      if (classGuidListChecked[i] == value.guidfixed) {
        isCheck = true;
        break;
      }
    }
          ...
          if (showCheckBox == true) {
            setState(() {
              ...
              global.showSnackBar(
                  ...
                  "${global.language("chosen")} ${classGuidListChecked.length} ${global.language("list")}",
                  Colors.blue);
            });
          } else {
            setState(() {
  ...
  bool verifyData(MasterClassModel value) {
  ...
  void saveOrUpdateData() {
    if (screenEvent == global.ScreenEventEnum.edit ||
      ...
      MasterClassModel classData = MasterClassModel(
        guidfixed: "",
        code: fieldTextController[0].text,
        names: packLanguage(),
      );
      if (verifyData(classData)) {
        showCheckBox = false;
        if (selectGuid.trim().isEmpty) {
          context
              .read<MasterClassBloc>()
              .add(MasterClassSave(classModel: classData));
        } else {
          context
              .read<MasterClassBloc>()
              .add(MasterClassUpdate(guid: selectGuid, classModel: classData));
        }
      }
    }
  }

  void getDataToEditScreen(MasterClassModel classData) {
    isChange = false;
    selectGuid = classData.guidfixed;
    fieldTextController[0].text = classData.code;
    for (int i = 0; i < languageList.length; i++) {
      fieldTextController[i + 1].text = "";
    }
    for (int i = 0; i < languageList.length; i++) {
      for (int j = 0; j < classData.names.length; j++) {
        if (languageList[i].code! == classData.names[j].code) {
          fieldTextController[i + 1].text = classData.names[j].name;
        }
      }
    }
  }
    ...
    return Scaffold(
        ...
        appBar: AppBar(
            ...
            leading: mobileScreen
                ...
                : null,
            title: Text(headerEdit + global.language("class")),
            actions: <Widget>[
                            ...
                            actions: <Widget>[
                              ...
                              ElevatedButton(
                                  ...
                                  onPressed: () {
                                    Navigator.pop(context);
                                    context.read<MasterClassBloc>().add(
                                        MasterClassDelete(guid: selectGuid));
                                  },
                                  child: Text(global.language('confirm'))),
                            ],
                          ),
                    ...
                    child: Column(children: [
                      ...
                      TextFormField(
                          ...
                          decoration: InputDecoration(
                            ...
                            labelText: global.language("class_code"),
                        ...
                        Padding(
                          ...
                          child: TextFormField(
                            ...
                            decoration: InputDecoration(
                              ...
                              border: const OutlineInputBorder(),
                              labelText:
                                  "${global.language("class_name")} (${languageList[i].name})",
        ...
        body: LayoutBuilder(builder: (context, constraints) {
          return BlocListener<MasterClassBloc, MasterClassState>(
              listener: (context, state) {
                blocMasterClassState = state;
                if (state is MasterClassLoadSuccess) {
                  setState(() {
                    if (state.classes.isNotEmpty) {
                      listData.addAll(state.classes);
                    }
                  });
                }
                if (state is MasterClassSaveSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterClassSaveFailed) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterClassUpdateSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterClassUpdateFailed) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterClassDeleteSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterClassDeleteManySuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is MasterClassGetSuccess) {
                  setState(() {
                    getDataToEditScreen(state.classData);
                    if (screenEvent == global.ScreenEventEnum.edit) {
                      WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
                        tabController.animateTo(1);
                      });

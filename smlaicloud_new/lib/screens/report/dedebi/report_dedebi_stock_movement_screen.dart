...
class ReportStockMovementScreen extends StatefulWidget {
  ...
  @override
  State<ReportStockMovementScreen> createState() => _ReportStockMovementScreenState();
}
...
class _ReportStockMovementScreenState extends State<ReportStockMovementScreen> {
  ...
  EntitySelectionModel _selectedBarcodes = const EntitySelectionModel(selectedEntities: [], isCancel: false);

  int _pageSize = 20;

  BiReportMeta? _currentMeta;
  List<StockMovementModel> _currentData = [];
  String _currentJobId = '';

  StockMovmentSummaryModel? _currentTotalSummary;
        ...
        appBar: AppBar(
          title: Text(
            _selectedBarcodes.selectedEntities.isNotEmpty
                ? 'เคลื่อนไหวของสต็อก : ${_selectedBarcodes.selectedEntities.first.code}~${ReportUtils.getDisplayNameSafe(_selectedBarcodes.selectedEntities.first.names)}'
                : 'เคลื่อนไหวของสต็อก',
  ...
  void _handleStateChanges(BiReportState state) {
    switch (state) {
      ...
      case BiReportSummarySuccess():
        setState(() {
          if (state.stockMovementSummaryData != null) {
            _currentTotalSummary = state.stockMovementSummaryData!;
          } else {
            print('⚠️ Warning: stockMovementSummaryData is null in BiReportSummarySuccess');
          }
  ...
  Widget _buildContent(BiReportState state) {
    return switch (state) {
      ...
      BiReportGenerateSuccess() => _buildSuccessContent(state.data.whereType<StockMovementModel>().toList(), state.jobId, state.meta),
      BiReportDetailLoading() => _buildContentWithLoading(state.jobId),
      BiReportDetailSuccess() => _buildSuccessContent(state.data.whereType<StockMovementModel>().toList(), state.jobId, state.meta),
  ...
  void _updateCurrentData(dynamic state) {
    if (state is BiReportGenerateSuccess || state is BiReportDetailSuccess) {
      setState(() {
        _currentData = state.data.whereType<StockMovementModel>().toList();
  ...
  void _requestTotalSummary(String jobId) {
    context.read<BiReportBloc>().add(
          GetBiReportSummaryRequested(
            reportType: BiReportType.stockMovement,
            jobId: jobId,
  ...
  Widget _buildSuccessContent(List<StockMovementModel> data, String jobId, BiReportMeta meta) {
    return _buildMainLayout(data, jobId, meta);
  }

  Widget _buildMainLayout(List<StockMovementModel> data, String jobId, BiReportMeta meta) {
    return Row(
  ...
  Widget _buildMainLayoutWithLoading(List<StockMovementModel> data, String jobId, BiReportMeta meta) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildSidebar(data),
  ...
  Widget _buildSidebar(List<StockMovementModel> data) {
    return Container(
      width: 260,
      ...
      child: Column(
        children: [
          ...
          TotalSummaryPanel(
            stockSummary: _currentTotalSummary,
  ...
  Widget _buildMainContent(List<StockMovementModel> data, String jobId, BiReportMeta meta) {
    return Container(
  ...
  Widget _buildDataGrid(List<StockMovementModel> data) {
    return Container(
  ...
  void _refreshReportData() {
    ...
    final conditions = ReportConditionsModel(
      ...
      barcode: _selectedBarcodes.selectedEntities.isNotEmpty ? (_selectedBarcodes.selectedEntities.first.code.toString()) : '',
    );
    ...
    print('   - reportType: ${BiReportType.stockMovement}');

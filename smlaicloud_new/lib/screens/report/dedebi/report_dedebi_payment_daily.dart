...
import 'package:smlaicloud/screens/report/dedebi/components/transaction_payment_daily_dialog.dart';
import 'package:smlaicloud/screens/report/dedebi/utils/report_utils.dart';
import 'package:smlaicloud/utils/date_picker.dart';

class ReportPaymentDailyScreen extends StatefulWidget {
  const ReportPaymentDailyScreen({super.key});

  @override
  State<ReportPaymentDailyScreen> createState() => _ReportPaymentDailyScreenState();
}
...
class _ReportPaymentDailyScreenState extends State<ReportPaymentDailyScreen> {
  ...
  List<PaymentDailyModel> _currentData = [];
  String _currentJobId = '';

  PaymentDailySummaryModel? _currentTotalSummary;
  ...
  void _handleStateChanges(BiReportState state) {
    switch (state) {
      ...
      case BiReportSummarySuccess():
        setState(() {
          if (state.paymentDailySummaryData != null) {
            _currentTotalSummary = state.paymentDailySummaryData!;
          } else {
  ...
  Widget _buildContent(BiReportState state) {
    return switch (state) {
      ...
      BiReportGenerateSuccess() => _buildSuccessContent(state.data.whereType<PaymentDailyModel>().toList(), state.jobId, state.meta),
      BiReportDetailLoading() => _buildContentWithLoading(state.jobId),
      BiReportDetailSuccess() => _buildSuccessContent(state.data.whereType<PaymentDailyModel>().toList(), state.jobId, state.meta),
  ...
  void _updateCurrentData(dynamic state) {
    if (state is BiReportGenerateSuccess || state is BiReportDetailSuccess) {
      setState(() {
        _currentData = state.data.whereType<PaymentDailyModel>().toList();
  ...
  Widget _buildSuccessContent(List<PaymentDailyModel> data, String jobId, BiReportMeta meta) {
    return _buildMainLayout(data, jobId, meta);
  }

  Widget _buildMainLayout(List<PaymentDailyModel> data, String jobId, BiReportMeta meta) {
    return Row(
  ...
  Widget _buildMainLayoutWithLoading(List<PaymentDailyModel> data, String jobId, BiReportMeta meta) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildSidebar(data),
  ...
  Widget _buildSidebar(List<PaymentDailyModel> data) {
    return Container(
      width: 260,
      ...
      child: Column(
        children: [
          ...
          TotalSummaryPanel(
            paymentSummary: _currentTotalSummary,
  ...
  Widget _buildMainContent(List<PaymentDailyModel> data, String jobId, BiReportMeta meta) {
    return Container(
  ...
  void _refreshReportData() {
    ...
    print('   - reportType: ${BiReportType.paymentDaily}');
  ...
  void _showTransactionDialog(PaymentDailyModel payment) {
    if (payment.transactions.isNotEmpty) {
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return TransactionPaymentDailyDialog(
            payment: payment,
            formatCurrency: ReportUtils.formatCurrency,
          );
        },

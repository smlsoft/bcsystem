...
class ReportStockBalanceScreen extends StatefulWidget {
  ...
  @override
  State<ReportStockBalanceScreen> createState() => _ReportStockBalanceScreenState();
}
...
class _ReportStockBalanceScreenState extends State<ReportStockBalanceScreen> {
        ...
        appBar: AppBar(
          title: Text(
            _selectedBarcodes.selectedEntities.isNotEmpty
                ? 'สินค้าคงเหลือ : ${_selectedBarcodes.selectedEntities.first.code}~${ReportUtils.getDisplayNameSafe(_selectedBarcodes.selectedEntities.first.names)}'
                : 'สินค้าคงเหลือ',
  ...
  Widget _buildContent(BiReportState state) {
    return switch (state) {
      ...
      BiReportGenerateSuccess() => _buildSuccessContent(state.data.whereType<StockBalanceModel>().toList(), state.jobId, state.meta),
      BiReportDetailLoading() => _buildContentWithLoading(state.jobId),
      BiReportDetailSuccess() => _buildSuccessContent(state.data.whereType<StockBalanceModel>().toList(), state.jobId, state.meta),
  ...
  Widget _buildSuccessContent(List<StockBalanceModel> data, String jobId, BiReportMeta meta) {
    return _buildMainLayout(data, jobId, meta);
  }

  Widget _buildMainLayout(List<StockBalanceModel> data, String jobId, BiReportMeta meta) {
    return Row(
  ...
  Widget _buildMainLayoutWithLoading(List<StockBalanceModel> data, String jobId, BiReportMeta meta) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildSidebar(data),
  ...
  Widget _buildMainContent(List<StockBalanceModel> data, String jobId, BiReportMeta meta) {
    return Container(
  ...
  void _refreshReportData() {
    ...
    print('   - reportType: ${BiReportType.stockBalance}');

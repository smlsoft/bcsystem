...
/// รายงานยอดขาย Dedebi ที่เชื่อมต่อกับ BiReportBloc
/// รองรับการแสดงความคืบหน้าและข้อมูลแบบเรียลไทม์
class ReportDedebiSalesScreen extends StatefulWidget {
  const ReportDedebiSalesScreen({super.key});

  @override
  State<ReportDedebiSalesScreen> createState() => _ReportDedebiSalesScreenState();
}
...
class _ReportDedebiSalesScreenState extends State<ReportDedebiSalesScreen> {
  ...
  String _showCancelledDocuments = 'ไม่แสดง';
  String _saleType = 'ทั้งหมด'; // ทั้งหมด (default)
  String _posType = 'ทั้งหมด'; // ทั้งหมด (default)
  BranchSelectionModel _selectedBranches = const BranchSelectionModel(selectedBranches: [], isCancel: false); // Branch selection
  EntitySelectionModel _selectedDebtors = const EntitySelectionModel(selectedEntities: [], isCancel: false); // Creditor selection
  EntitySelectionModel _selectedSalespersons = const EntitySelectionModel(selectedEntities: [], isCancel: false); // Employee selection
  ...
  List<SaleReportData> _currentData = [];
  String _currentJobId = ''; // Add this to track current job ID

  // Total summary data
  SaleReportSummary? _currentTotalSummary;
  ...
  @override
  void initState() {
    print('🚀 ReportDedebiSalesScreen initState called');
    super.initState();
  ...
  void _handleStateChanges(BiReportState state) {
    switch (state) {
      ...
      case BiReportSummarySuccess():
        setState(() {
          _currentTotalSummary = state.summaryData;
  ...
  Widget _buildContent(BiReportState state) {
    return switch (state) {
      ...
      BiReportGenerateSuccess() => _buildSuccessContent(state.data.whereType<SaleReportData>().toList(), state.jobId, state.meta),
      BiReportDetailLoading() => _buildContentWithLoading(state.jobId),
      BiReportDetailSuccess() => _buildSuccessContent(state.data.whereType<SaleReportData>().toList(), state.jobId, state.meta),
  ...
  void _updateCurrentData(dynamic state) {
    if (state is BiReportGenerateSuccess || state is BiReportDetailSuccess) {
      setState(() {
        _currentData = state.data.whereType<SaleReportData>().toList();
  ...
  Widget _buildSuccessContent(List<SaleReportData> data, String jobId, BiReportMeta meta) {
    return _buildMainLayout(data, jobId, meta);
  }

  Widget _buildMainLayout(List<SaleReportData> data, String jobId, BiReportMeta meta) {
    return Row(
  ...
  Widget _buildMainLayoutWithLoading(List<SaleReportData> data, String jobId, BiReportMeta meta) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildSidebar(data),
  ...
  Widget _buildSidebar(List<SaleReportData> data) {
    return Container(
      width: 260,
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            TotalSummaryPanel(
              totalSummary: _currentTotalSummary,
  ...
  Widget _buildMainContent(List<SaleReportData> data, String jobId, BiReportMeta meta) {
    return Container(
  ...
  Widget _buildDataGrid(List<SaleReportData> data) {
    return Container(
      ...
      child: SaleTableView(
        ...
        onRowTap: _showTransactionDialog,
        reportType: BiReportType.sale,
      ),
    );
  }
    ...
    await showDialog(
      ...
      builder: (BuildContext context) {
        return ReportConditionDialog(
          ...
          initialSaleType: _saleType,
          initialPosType: _posType,
          initialSelectedBranches: _selectedBranches,
          initialSelectedCreditors: _selectedDebtors, // เปลี่ยนเป็น _selectedDebtors
          initialSelectedSalespersons: _selectedSalespersons,
          onConditionsSet: (conditions) {
            ...
            setState(() {
              ...
              _saleType = conditions.saleType!;
              _posType = conditions.posType!;
              _selectedBranches = conditions.selectedBranches!;
              _selectedDebtors = conditions.selectedCreditors!; // เปลี่ยนเป็น _selectedDebtors
              _selectedSalespersons = conditions.selectedSalespersons!;
            });
  ...
  void _refreshReportData() {
    ...
    final conditions = ReportConditionsModel(
      ...
      iscancel: (_showCancelledDocuments == 'ทั้งหมด' || _showCancelledDocuments == '') ? '' : (_showCancelledDocuments == 'แสดง' ? 'true' : 'false'),
      inquirytype: (_saleType == 'ทั้งหมด' || _saleType == '') ? '' : (_saleType == 'ขาย' ? '1' : '2'),
      ispos: _posType == 'POS เท่านั้น',
      creditorcode: _selectedDebtors.getEntityCodeString(), // ใช้ _selectedDebtors แทน
    ...
    print('   - reportType: ${BiReportType.sale}');
  ...
  void _showTransactionDialog(SaleReportData sale) {

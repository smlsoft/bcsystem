...
class ReportDedebiSaleDailyScreen extends StatefulWidget {
  const ReportDedebiSaleDailyScreen({super.key});

  @override
  State<ReportDedebiSaleDailyScreen> createState() => _ReportDedebiSaleDailyScreenState();
}

class _ReportDedebiSaleDailyScreenState extends State<ReportDedebiSaleDailyScreen> {
  ...
  String _showCancelledDocuments = 'ทั้งหมด';
  ...
  List<SaleDailyReportData> _currentData = [];
  String _currentJobId = ''; // Add this to track current job ID

  // Total summary data
  SaleDailyReportSummary? _currentTotalSummary;
  ...
  void _handleStateChanges(BiReportState state) {
    switch (state) {
      ...
      case BiReportSummarySuccess():
        setState(() {
          // ตรวจสอบก่อนว่าเป็น daily summary จริงๆ
          if (state.dailySummaryData != null) {
            _currentTotalSummary = state.dailySummaryData;
          } else {
            print('⚠️ Warning: dailySummaryData is null in BiReportSummarySuccess');
          }
  ...
  Widget _buildContent(BiReportState state) {
    return switch (state) {
      ...
      BiReportGenerateSuccess() => _buildSuccessContent(state.data.whereType<SaleDailyReportData>().toList(), state.jobId, state.meta),
      BiReportDetailLoading() => _buildContentWithLoading(state.jobId),
      BiReportDetailSuccess() => _buildSuccessContent(state.data.whereType<SaleDailyReportData>().toList(), state.jobId, state.meta),
  ...
  void _updateCurrentData(dynamic state) {
    if (state is BiReportGenerateSuccess || state is BiReportDetailSuccess) {
      setState(() {
        _currentData = state.data.whereType<SaleDailyReportData>().toList();
  ...
  Widget _buildSuccessContent(List<SaleDailyReportData> data, String jobId, BiReportMeta meta) {
    return _buildMainLayout(data, jobId, meta);
  }

  Widget _buildMainLayout(List<SaleDailyReportData> data, String jobId, BiReportMeta meta) {
    return Row(
  ...
  Widget _buildMainLayoutWithLoading(List<SaleDailyReportData> data, String jobId, BiReportMeta meta) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildSidebar(data),
  ...
  Widget _buildSidebar(List<SaleDailyReportData> data) {
    return Container(
      width: 260,
      ...
      child: Column(
        children: [
          ...
          TotalSummaryPanel(
            dailySummary: _currentTotalSummary,
  ...
  Widget _buildMainContent(List<SaleDailyReportData> data, String jobId, BiReportMeta meta) {
    return Container(
  ...
  Widget _buildDataGrid(List<SaleDailyReportData> data) {
    return Container(
      ...
      child: SaleTableView(
        ...
        onRowSaleDailyTap: _showTransactionDialog,
        reportType: BiReportType.saleDaily,
      ),
    );
  }
  ...
  void _refreshReportData() {
    ...
    print('   - reportType: ${BiReportType.saleDaily}');
  ...
  void _showTransactionDialog(SaleDailyReportData sale) {
    if (sale.transactions.isNotEmpty) {
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return TransactionDailyDetailDialog(
            sale: sale,
            formatCurrency: ReportUtils.formatCurrency,
          );
        },

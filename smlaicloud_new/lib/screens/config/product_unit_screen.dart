...
class ProductUnitScreenState extends State<ProductUnitScreen> with SingleTickerProviderStateMixin {
  ...
  // ฟังก์ชันสำหรับเลือกไฟล์ Excel และอัปโหลด
  void selectAndUploadExcelFile() async {
    try {
      ...
      if (result != null) {
        // ใช้ bytes สำหรับทั้ง web และ mobile/desktop
        if (result.files.single.bytes != null) {
          // ส่งไฟล์ไปยัง API ผ่าน Bloc โดยใช้ bytes
          context.read<UnitBloc>().add(UnitUploadExcel(
      ...
      global.showSnackBar(
        ...
        "นำเข้าไม่สำเร็จ: ${e.toString()}",
        Colors.red,
      );
    }
  }

  // ฟังก์ชันแสดงผลลัพธ์การอัปโหลด
  void showUploadResult(String resultMessage) {
    String message = "";
    Color color = Colors.green;
    Icon icon = const Icon(Icons.check_circle, color: Colors.white);

    if (resultMessage.contains('Existing unit codes: []')) {
      message = "นำเข้าสำเร็จ";
    } else if (resultMessage.contains('Existing unit codes:')) {
      // แยกรายการ unit codes ที่ซ้ำ
      String existingCodes = resultMessage.replaceAll('Existing unit codes: ', '');
      message = "นำเข้าสำเร็จ - หน่วยนับที่มีอยู่แล้ว: $existingCodes";
      color = Colors.orange;
      icon = const Icon(Icons.warning, color: Colors.white);
    } else {
      message = "นำเข้าสำเร็จ: $resultMessage";
    }

    global.showSnackBar(context, icon, message, color);
        ...
        actions: <Widget>[
          Padding(
            ...
            child: IconButton(
              focusNode: FocusNode(skipTraversal: true),
              onPressed: () async {
                // Get shop info from local storage using ShopModel
                ...
                if (shopInfoJson != null && shopInfoJson.isNotEmpty) {
                  try {
                    ...
                    if (mainShopId.isNotEmpty) {
                      // ignore: use_build_context_synchronously
                      context.read<UnitBloc>().add(UnitLoadFromMainShop(mainShopId: mainShopId));
                      return;
                    }
                  } catch (e) {
                    // If error parsing shop info, fall back to template
                    print("Error parsing shop info: $e");
                  }
                }

                // Fall back to template (productcentertype = 0 or error)
                await getTemplate();
              ...
              tooltip: global.language('template_download'),
            ),
          ),
          Padding(
            padding: const EdgeInsets.only(right: 20.0),
            child: IconButton(
              ...
              onPressed: () {
                try {
                  downloadAssetFile('assets/file_import/import_unit.xlsx', 'import_unit_template.xlsx');
                } catch (e) {
                  global.showSnackBar(
                    context,
                    const Icon(Icons.error, color: Colors.white),
                ...
                child: Container(
                    ...
                    child: Column(children: [
                      ...
                      TextFormField(
                        ...
                        onChanged: (value) {
                          isChange = true;
                          fieldTextController[0].value = TextEditingValue(text: value, selection: fieldTextController[0].selection);
                        },
                        decoration: InputDecoration(
        ...
        body: LayoutBuilder(builder: (context, constraints) {
          return BlocListener<UnitBloc, UnitState>(
              listener: (context, state) {
                ...
                // Load from Main Shop
                if (state is UnitLoadFromMainShopSuccess) {
                  setState(() {
                    listTempData = state.units;

                    // Remove duplicates - if unit already exists in listData, remove from listTempData
                    if (listData.isNotEmpty) {
                      for (int i = 0; i < listData.length; i++) {
                        listTempData.removeWhere((element) => element.unitcode == listData[i].unitcode);
                      }
                  ...
                  // Show dialog with main shop units
                  // ignore: use_build_context_synchronously
                  DialogTemplate.showDataListTemplateDialog(context, listTempData, "unit").then((selectedData) {
                    if (selectedData.isNotEmpty) {
                      List<UnitModel> unitTempSeleted = [];
                      for (int i = 0; i < selectedData.length; i++) {
                        if (selectedData[i] == true) {
                      ...
                      if (unitTempSeleted.isNotEmpty) {
                        context.read<UnitBloc>().add(UnitSaveBulk(units: unitTempSeleted));
                      }
                    }
                  });
                }
                if (state is UnitLoadFromMainShopFailed) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                        ...
                        "ไม่สามารถโหลดข้อมูลจากร้านหลักได้: ${state.message}",
                        Colors.red);
                  });
                }
                // Excel Upload
                if (state is UnitUploadExcelSuccess) {
                  setState(() {
                    showUploadResult(state.uploadResult);
                  });
                }
                if (state is UnitUploadExcelFailed) {
                  setState(() {
                    global.showSnackBar(context, const Icon(Icons.error, color: Colors.white), "นำเข้าไม่สำเร็จ: ${state.message}", Colors.red);
                  });
                }
                // Get

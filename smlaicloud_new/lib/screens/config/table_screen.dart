...
class TableScreen extends StatefulWidget {
  ...
  const TableScreen({Key? key, required this.groupnumber}) : super(key: key);

  @override
  State<TableScreen> createState() => TableScreenState();
}

class TableScreenState extends State<TableScreen> with SingleTickerProviderStateMixin {
  final translator = GoogleTranslator();
  late TabController tabController;
  ScrollController editScrollController = ScrollController();
  TextEditingController searchController = TextEditingController();
  List<LanguageModel> languageList = <LanguageModel>[];
  List<TableModel> listData = [];
  ...
  late TableState blocCurrentState;
  String headerEdit = "";
  ...
  late ZoneDataModel zoneSelected;
  ...
  void loadDataZoneList() {
    context.read<ZoneBloc>().add(ZoneLoadList(offset: 0, limit: 1000, search: "", groupNumber: screenData.zonenumber!));
  }
  ...
  void clearEditData() {
    ...
    screenData = TableModel(guidfixed: '', names: [], number: '', zone: '', groupnumber: widget.groupnumber, zonenumber: 1);
    zoneSelected = ZoneDataModel(code: '', names: [], guidfixed: '', groupnumber: screenData.zonenumber);
    isDataChange = false;
  }

  void loadDataToScreen() {
    if (screenData.zonenumber == 0) {
      screenData.zonenumber = 1;
    }

    codeController.text = screenData.number;
    zoneSelected = listDataZone.firstWhere((element) => element.code == screenData.zone, orElse: () => ZoneDataModel(code: '', names: [], guidfixed: ''));
  }
  ...
  void getData(String guid) {
    ...
    isEditMode = false;
    context.read<TableBloc>().add(TableGet(guid: guid));
  }

  void switchToEdit(TableModel value) {
    setState(() {
  ...
  Widget listScreen({bool mobileScreen = false}) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("${global.language("Table")} ${widget.groupnumber}"),
        leading: IconButton(
                      ...
                      builder: (BuildContext context) => AlertDialog(
                        ...
                        actions: <Widget>[
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () {
                                Navigator.pop(context);
                                context.read<TableBloc>().add(TableDeleteMany(guid: guidListChecked));
                              },
        ...
        child: Column(children: [
          ...
          Container(
              ...
              child: Row(children: [
                Expanded(
                    flex: 5,
                    child: Text(global.language("Table_code"),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            Expanded(flex: 5, child: Text(value.number, maxLines: 2, overflow: TextOverflow.ellipsis, style: textStyle)),
            Expanded(flex: 5, child: Text(global.packName(value.names), maxLines: 2, overflow: TextOverflow.ellipsis, style: textStyle)),
            if (showCheckBox)
              Expanded(
                flex: 1,
                child: (isCheck) ? Icon(Icons.check, size: global.deviceConfig.listDataFontSize) : Container(),
              )
          ],
        ),
      ),
  ...
  void saveOrUpdateData() {
    showCheckBox = false;
    screenData.number = codeController.text;
    screenData.zone = zoneSelected.code;

    if (selectGuid.trim().isEmpty) {
      context.read<TableBloc>().add(TableSave(tableModel: screenData));
    } else {
  ...
  void updateData(String guid) {
    ...
    context.read<TableBloc>().add(TableUpdate(guid: guid, tableModel: screenData));
  }
  ...
  Widget editScreen({mobileScreen}) {
    ...
    for (int languageIndex = 0; languageIndex < languageList.length; languageIndex++) {
      LanguageDataModel creditorName = screenData.names.firstWhere((element) => element.code == languageList[languageIndex].code, orElse: () => LanguageDataModel(code: '', name: ''));
      ...
      formWidgets.add(Padding(
        ...
        child: TextFormField(
          ...
          decoration: InputDecoration(
            floatingLabelBehavior: FloatingLabelBehavior.always,
            border: const OutlineInputBorder(),
            labelText: "${global.language("Table_name")} (${getLangName(screenData.names[languageIndex].code)})",
          ),
          validator: (value) {
            if (languageIndex == 0) {
              if (value == null || value.isEmpty) {
      ...
      Padding(
        ...
        child: Row(
          children: [
            ...
            /* XXXX
            Expanded(
              child: DropdownSearch<ZoneDataModel>(
                enabled: (isEditMode),
    ...
    return Scaffold(
        ...
        appBar: AppBar(
            ...
            leading: mobileScreen
                ...
                : null,
            title: Text("$headerEdit${global.language("Table")} ${widget.groupnumber}"),
            actions: <Widget>[
                          ...
                          builder: (BuildContext context) => AlertDialog(
                            ...
                            actions: <Widget>[
                              ...
                              ElevatedButton(
                                  ...
                                  onPressed: () {
                                    Navigator.pop(context);
                                    context.read<TableBloc>().add(TableDelete(guid: selectGuid));
                                  },
            ...
            listeners: [
              ...
              BlocListener<TableBloc, TableState>(
                listener: (context, state) {
                  ...
                  if (state is TableLoadSuccess) {
                    setState(() {
                      ...
                      if (state.tables.isNotEmpty) {
                        listData.addAll(state.tables);
                      }
                    });
                  }
                  if (state is TableLoadFailed) {
                    setState(() {
                      loadingData = false;
                    });
                  }
                  // Save
                  if (state is TableSaveSuccess) {
                    setState(() {
                      global.showSnackBar(
                          context,
                          const Icon(
                  ...
                  // Delete Many
                  if (state is TableDeleteManySuccess) {
                    setState(() {
                      global.showSnackBar(
                          context,
                          const Icon(
                  ...
                  // Get
                  if (state is TableGetSuccess) {
                    setState(() {
                      clearEditData();
                      isDataChange = false;
                      screenData = state.table;
                      loadDataToScreen();

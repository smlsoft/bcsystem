...
import 'package:smlaicloud/bloc/devices/devices_bloc.dart';
...
class DevicesScreenState extends State<DevicesScreen> with SingleTickerProviderStateMixin {
  ...
  late DevicesState blocDevicesState;
  String headerEdit = "";
  ...
  void loadDataList(String search) {
    context.read<DevicesBloc>().add(DevicesLoadList(offset: (listDatas.isEmpty) ? 0 : listDatas.length, limit: global.loadDataPerPage, search: search));
  }
  ...
  void getData(String guid) {
    ...
    isEditMode = false;
    context.read<DevicesBloc>().add(DevicesGet(guid: guid));
  }
                      ...
                      builder: (BuildContext context) => AlertDialog(
                        ...
                        actions: <Widget>[
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () {
                                Navigator.pop(context);
                                context.read<DevicesBloc>().add(DevicesDeleteMany(guid: guidListChecked));
                              },
            ...
            if (kIsWeb) {
              if (event is RawKeyDownEvent) {
                if (event.logicalKey == LogicalKeyboardKey.arrowUp) {
                  isKeyDown = false;
                  int index = listDatas.indexOf(listDatas.firstWhere((element) => element.guidfixed == selectGuid));
                  if (index > 0) {
          ...
          child: Column(
            children: [
              ...
              Container(
                  ...
                  child: Row(children: [
                    Expanded(flex: 5, child: Text(global.language("Devices_code"), style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold))),
                    Expanded(
                        flex: 10,
                        child: Text(
              ...
              Expanded(child: SingleChildScrollView(controller: listScrollController, child: Column(children: listDatas.map((value) => listObject(value, showCheckBox)).toList())))
            ],
          )),
    );
  }
  ...
  Widget listObject(DevicesModel value, bool showCheckBox) {
    bool isCheck = false;
  ...
  void saveOrUpdateData() {
    ...
    if (selectGuid.trim().isEmpty) {
      if (screenData.code != "" && screenData.name1 != "") {
        context.read<DevicesBloc>().add(DevicesSave(devices: screenData));
      } else {
        global.showSnackBar(
            context,
            const Icon(
  ...
  void updateData(String guid) {
    ...
    if (screenData.code != "" && screenData.name1 != "") {
      context.read<DevicesBloc>().add(DevicesUpdate(guid: guid, devices: screenData));
    } else {
      global.showSnackBar(
          context,
          const Icon(
  ...
  void getDataToEditScreen(DevicesModel devices) {
    isChange = false;
    selectGuid = devices.guidfixed;
    screenData.code = devices.code;
    screenData.name1 = devices.name1;
    screenData.guidfixed = devices.guidfixed;
    screenData.type = devices.type;
  }
    ...
    formWidgets.add(Padding(
        ...
        child: TextFormField(
            ...
            decoration: InputDecoration(
              ...
              labelText: global.language("Devices_code"),
            ))));
    nodeIndex++;
    formWidgets.add(Padding(
      padding: const EdgeInsets.only(bottom: 10),
      child: TextFormField(
          ...
          decoration: InputDecoration(
            ...
            contentPadding: const EdgeInsets.only(left: 10, top: 0, bottom: 0, right: 10),
            floatingLabelBehavior: FloatingLabelBehavior.always,
            labelText: global.language("Devices_name"),
          )),
    ));
    nodeIndex++;
    formWidgets.add(Padding(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Text(global.language("customer_display"))
              ],
            ),
            Row(
              mainAxisAlignment: MainAxisAlignment.start,
              children: [
                ...
                Text(global.language("chef_que_display"))
              ],
            ),
            Row(
              mainAxisAlignment: MainAxisAlignment.start,
              children: [
                ...
                Text(global.language("kitchen_display"))
              ],
            ),
          ],
        )));
                          ...
                          builder: (BuildContext context) => AlertDialog(
                            ...
                            actions: <Widget>[
                              ...
                              ElevatedButton(
                                  ...
                                  onPressed: () {
                                    Navigator.pop(context);
                                    context.read<DevicesBloc>().add(DevicesDelete(guid: selectGuid));
                                  },
              ...
              if (isSaveAllow == false && selectGuid.trim().isNotEmpty)
                Padding(
                    ...
                    child: IconButton(
                      ...
                      onPressed: () {
                        showCheckBox = false;
                        switchToEdit(listDatas[listDatas.indexOf(listDatas.firstWhere((element) => element.guidfixed == selectGuid))]);
                      },
                      icon: const Icon(
                        Icons.edit,
                      ),
        ...
        body: LayoutBuilder(builder: (context, constraints) {
          return BlocListener<DevicesBloc, DevicesState>(
              listener: (context, state) {
                blocDevicesState = state;
                // Load
                if (state is DevicesLoadSuccess) {
                  setState(() {
                    if (state.devices.isNotEmpty) {
                      listDatas.addAll(state.devices);
                    }
                  });
                }
                // Save
                if (state is DevicesSaveSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                // Update
                if (state is DevicesUpdateSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is DevicesUpdateFailed) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                // Delete Many
                if (state is DevicesDeleteManySuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                // Get
                if (state is DevicesGetSuccess) {
                  setState(() {
                    getDataToEditScreen(state.devices);
                    if (isEditMode) {
                      WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
                        tabController.animateTo(1);
                      });

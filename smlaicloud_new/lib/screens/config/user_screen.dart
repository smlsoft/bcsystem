...
class UserScreenState extends State<UserScreen> with SingleTickerProviderStateMixin {
  ...
  void loadDataList(String search) {
    ...
    searchText = search;
    context.read<UserBloc>().add(UserLoadList(offset: (listData.isEmpty) ? 0 : listData.length, limit: global.loadDataPerPage, search: search));
  }
          ...
          if (kIsWeb) {
            if (event is RawKeyDownEvent) {
              if (event.logicalKey == LogicalKeyboardKey.arrowUp) {
                isKeyDown = false;
                int index = listData.indexOf(listData.firstWhere((element) => element.username == username));
                if (index > 0) {
        ...
        child: Column(children: [
          ...
          Container(
              ...
              child: Row(children: [
                Expanded(
                    flex: 5,
                    child: Text(global.language("user_name"),
    ...
    return GestureDetector(
        ...
        child: Container(
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Expanded(flex: 5, child: Text(value.username, maxLines: 2, overflow: TextOverflow.ellipsis, style: textStyle)),
              Expanded(
                  flex: 5,
                  child: Text(
                      (value.role == 0
  ...
  Widget editScreen({mobileScreen}) {
    ...
    formWidgets.add(
      Padding(
        ...
        child: TextFormField(
          enabled: (appConfig.getString("createdby") == screenData.username) ? false : true,
          readOnly: !isEditMode,
          onChanged: (value) {
            isDataChange = true;
          },
        ...
        child: Row(
          children: [
            Radio(
              ...
              value: 0,
              groupValue: userRole,
              onChanged: ((appConfig.getString("createdby") != screenData.username))
                  ? (value) {
                      setState(() {
                      ...
                      builder: (BuildContext context) => AlertDialog(
                        ...
                        actions: <Widget>[
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () {
                                Navigator.pop(context);
                                context.read<UserBloc>().add(UserDelete(username: username));
                              },
              ...
              if (isSaveAllow == false && username.trim().isNotEmpty)
                Padding(
                    ...
                    child: IconButton(
                      focusNode: FocusNode(skipTraversal: true),
                      onPressed: () {
                        switchToEdit(listData[listData.indexOf(listData.firstWhere((element) => element.username == username))]);
                      },
                      icon: const Icon(
                        Icons.edit,
                      ),

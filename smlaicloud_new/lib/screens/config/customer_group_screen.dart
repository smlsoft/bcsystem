...
class CustomerGroupScreenState extends State<CustomerGroupScreen> with SingleTickerProviderStateMixin {
  ...
  List<CustomerGroupModel> listData = [];
  CustomerGroupModel dataTemp = CustomerGroupModel(guidfixed: '', groupcode: '');
  List<String> CustomerGroupGuidListChecked = [];
  ...
  void loadDataList(String search) {
    context.read<CustomerGroupBloc>().add(CustomerGroupLoadList(offset: (listData.isEmpty) ? 0 : listData.length, limit: global.loadDataPerPage, search: search));
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: <Widget>[
                ...
                onPressed: () {
                  discardData(callBack: () {
                    setState(() {
                      if (showCheckBox) {
                        showCheckBox = false;
                        CustomerGroupGuidListChecked.clear();
                      } else {
                        showCheckBox = true;
                        global.showSnackBar(
                            context,
          ...
          if (CustomerGroupGuidListChecked.isNotEmpty)
            Padding(
                padding: const EdgeInsets.only(right: 20.0),
                child: IconButton(
                  focusNode: FocusNode(skipTraversal: true),
                      ...
                      builder: (BuildContext context) => AlertDialog(
                        ...
                        actions: <Widget>[
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () {
                                Navigator.pop(context);
                                context.read<CustomerGroupBloc>().add(CustomerGroupDeleteMany(guid: CustomerGroupGuidListChecked));
                              },
  ...
  Widget listObject(int index, CustomerGroupModel value, bool showCheckBox) {
    bool isCheck = false;
    for (int i = 0; i < CustomerGroupGuidListChecked.length; i++) {
      if (CustomerGroupGuidListChecked[i] == value.guidfixed) {
        isCheck = true;
        break;
      }
    }
        ...
        onTap: () {
          if (showCheckBox == true) {
            setState(() {
              selectGuid = value.guidfixed;
              if (isCheck == true) {
                CustomerGroupGuidListChecked.remove(value.guidfixed);
              } else {
                CustomerGroupGuidListChecked.add(value.guidfixed);
              }
              global.showSnackBar(
                  context,
                  const Icon(
                  ...
                  "${global.language("chosen")} ${CustomerGroupGuidListChecked.length} ${global.language("list")}",
                  Colors.blue);
            });
          } else {
            setState(() {
  ...
  bool verifyData(CustomerGroupModel value) {
  ...
  void saveOrUpdateData() {
    if (screenEvent == global.ScreenEventEnum.edit || screenEvent == global.ScreenEventEnum.add) {
      CustomerGroupModel customerGroupModel = CustomerGroupModel(
        guidfixed: "",
        groupcode: fieldTextController[0].text,
        names: packLanguage(),
      );
      if (verifyData(customerGroupModel)) {
        showCheckBox = false;
        if (selectGuid.trim().isEmpty) {
          context.read<CustomerGroupBloc>().add(CustomerGroupSave(customerGroup: customerGroupModel));
        } else {
          showCheckBox = false;
          context.read<CustomerGroupBloc>().add(CustomerGroupUpdate(guid: selectGuid, customerGroup: customerGroupModel));
        }
      }
    }
  }

  void getDataToEditScreen(CustomerGroupModel customerGroupModel) {
    isChange = false;
    dataTemp = customerGroupModel;
    selectGuid = customerGroupModel.guidfixed;
    fieldTextController[0].text = customerGroupModel.groupcode;
    for (int i = 0; i < languageList.length; i++) {
      fieldTextController[i + 1].text = "";
    }
    for (int i = 0; i < languageList.length; i++) {
      for (int j = 0; j < customerGroupModel.names.length; j++) {
        if (languageList[i].code! == customerGroupModel.names[j].code) {
          fieldTextController[i + 1].text = customerGroupModel.names[j].name;
        }
      }
    }
  }
                          ...
                          builder: (BuildContext context) => AlertDialog(
                            ...
                            actions: <Widget>[
                              ...
                              ElevatedButton(
                                  ...
                                  onPressed: () {
                                    Navigator.pop(context);
                                    context.read<CustomerGroupBloc>().add(CustomerGroupDelete(guid: selectGuid));
                                  },
                                  child: Text(global.language('confirm'))),
                            ],
                          ),
                        ...
                        Padding(
                          ...
                          child: TextFormField(
                            ...
                            decoration: InputDecoration(
                              ...
                              border: const OutlineInputBorder(),
                              labelText: "${global.language("customer_name")} (${languageList[i].name})",
                            ),
                          ),
                        ),
                      if (isSaveAllow)
                ...
                if (state is CustomerGroupLoadSuccess) {
                  setState(() {
                    if (state.customerGroups.isNotEmpty) {
                      listData.addAll(state.customerGroups);
                    }
                  });
                }
                // Save

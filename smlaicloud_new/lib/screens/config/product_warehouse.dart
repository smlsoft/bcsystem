...
class ProductWarehouseScreen extends StatefulWidget {
  ...
  @override
  State<ProductWarehouseScreen> createState() => ProductWarehouseScreenState();
}

class ProductWarehouseScreenState extends State<ProductWarehouseScreen> with SingleTickerProviderStateMixin {
  ...
  List<WarehouseModel> listData = [];
  List<String> warehouseGuidListChecked = [];
  ...
  void loadDataList(String search) {
    context.read<WarehouseBloc>().add(WarehouseLoadList(offset: (listData.isEmpty) ? 0 : listData.length, limit: global.loadDataPerPage, search: search));
  }
  ...
  Widget listScreen({bool mobileScreen = false}) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(global.language('product_warehouse')),
        leading: IconButton(
        ...
        actions: <Widget>[
          ...
          if (warehouseGuidListChecked.isNotEmpty)
            Padding(
                padding: const EdgeInsets.only(right: 20.0),
                child: IconButton(
                  focusNode: FocusNode(skipTraversal: true),
                      ...
                      builder: (BuildContext context) => AlertDialog(
                        ...
                        actions: <Widget>[
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () {
                                Navigator.pop(context);
                                context.read<WarehouseBloc>().add(WarehouseDeleteMany(guid: warehouseGuidListChecked));
                              },
  ...
  void switchToEdit(WarehouseModel value) {
    setState(() {
  ...
  Widget listObject(int index, WarehouseModel value, bool showCheckBox) {
    bool isCheck = false;
    for (int i = 0; i < warehouseGuidListChecked.length; i++) {
      if (warehouseGuidListChecked[i] == value.guidfixed) {
        isCheck = true;
        break;
      }
    }
        ...
        onTap: () {
          if (showCheckBox == true) {
            setState(() {
              selectGuid = value.guidfixed;
              if (isCheck == true) {
                warehouseGuidListChecked.remove(value.guidfixed);
              } else {
                warehouseGuidListChecked.add(value.guidfixed);
              }
              global.showSnackBar(
                  context,
                  const Icon(
                  ...
                  "${global.language("chosen")} ${warehouseGuidListChecked.length} ${global.language("list")}",
                  Colors.blue);
            });
          } else {
            setState(() {
  ...
  bool verifyData(WarehouseModel value) {
  ...
  void saveOrUpdateData() {
    if (screenEvent == global.ScreenEventEnum.edit || screenEvent == global.ScreenEventEnum.add) {
      ...
      if (verifyData(warehouse)) {
        showCheckBox = false;
        if (selectGuid.trim().isEmpty) {
          context.read<WarehouseBloc>().add(WarehouseSave(warehouseModel: warehouse));
        } else {
          context.read<WarehouseBloc>().add(WarehouseUpdate(guid: selectGuid, warehouseModel: warehouse));
        }
      }
    }
  }

  void getDataToEditScreen(WarehouseModel warehouse) {
    isChange = false;
    selectGuid = warehouse.guidfixed;
    fieldTextController[0].text = warehouse.code;
    for (int i = 0; i < languageList.length; i++) {
      fieldTextController[i + 1].text = "";
    }
    for (int i = 0; i < languageList.length; i++) {
      for (int j = 0; j < warehouse.names.length; j++) {
        if (languageList[i].code! == warehouse.names[j].code) {
          fieldTextController[i + 1].text = warehouse.names[j].name;
        }
      }
    }
  }
                          ...
                          builder: (BuildContext context) => AlertDialog(
                            ...
                            actions: <Widget>[
                              ...
                              ElevatedButton(
                                  ...
                                  onPressed: () {
                                    Navigator.pop(context);
                                    context.read<WarehouseBloc>().add(WarehouseDelete(guid: selectGuid));
                                  },
                                  child: Text(global.language('confirm'))),
                            ],
                          ),
                        ...
                        Padding(
                          ...
                          child: TextFormField(
                            ...
                            decoration: InputDecoration(
                              ...
                              border: const OutlineInputBorder(),
                              labelText: "${global.language("warehouse_name")} (${languageList[i].name})",
        ...
        body: LayoutBuilder(builder: (context, constraints) {
          return BlocListener<WarehouseBloc, WarehouseState>(
              listener: (context, state) {
                ...
                // Save
                if (state is WarehouseSaveSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is WarehouseSaveFailed) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                // Delete
                if (state is WarehouseDeleteSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                // Delete Many
                if (state is WarehouseDeleteManySuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(

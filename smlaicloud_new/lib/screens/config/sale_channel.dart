...
class SaleChannelScreen extends StatefulWidget {
  const SaleChannelScreen({super.key});

  @override
  State<SaleChannelScreen> createState() => SaleChannelScreenState();
}

class SaleChannelScreenState extends State<SaleChannelScreen> with SingleTickerProviderStateMixin {
  ...
  List<SaleChannelModel> listTempData = [];
  ...
  late SaleChannelState blocCurrentState;
  String headerEdit = "";
  ...
  /// เลือกเฉพาะ ราคา เดลิเวอรี่
  final List<PriceModel> priceList = (global.config.prices != null) 
      ? global.config.prices.where((element) => element.keyNumber >= 3).toList()
      : [];
  ...
  Future<void> getTemplate() async {
    const githubRawUrl = 'https://raw.githubusercontent.com/smlsoft/dedepos_template/main/delivery.json';
    try {
      final fileContent = await global.readFileFromGithub(githubRawUrl);
      final salechannel = (json.decode(fileContent) as List).map((salechannel) => SaleChannelModel.fromJson(salechannel)).toList();
      listTempData = [];

      for (int i = 0; i < salechannel.length; i++) {
        listTempData.add(salechannel[i]);
      }
  ...
  void getData(String guid) {
    ...
    imageWeb = null;
    imageFile = File('');
    context.read<SaleChannelBloc>().add(SaleChannelGet(guid: guid));
  }

  void switchToEdit(SaleChannelModel value) {
    setState(() {
          ...
          Padding(
            ...
            child: IconButton(
              ...
              onPressed: () async {
                ...
                List<dynamic> selectedData = await DialogTemplate.showDataListTemplateDialog(context, listTempData, "salechannel");

                if (selectedData.isNotEmpty) {
                  List<SaleChannelModel> saleChannelTempSeleted = [];
                  for (int i = 0; i < selectedData.length; i++) {
                    if (selectedData[i] == true) {
                      saleChannelTempSeleted.add(listTempData[i]);
                    }
                  }

                  // ignore: use_build_context_synchronously
                  context.read<SaleChannelBloc>().add(SaleChannelSaveBulk(salechannels: saleChannelTempSeleted));
                }
              },
                      ...
                      builder: (BuildContext context) => AlertDialog(
                        ...
                        actions: <Widget>[
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () {
                                Navigator.pop(context);
                                context.read<SaleChannelBloc>().add(SaleChannelDeleteMany(guid: guidListChecked));
                              },
        ...
        child: Column(children: [
          ...
          Container(
              ...
              child: Row(children: [
                Expanded(
                    flex: 5,
                    child: Text(global.language("sale_channel_code"),
  ...
  Widget listObject(int index, SaleChannelModel value, bool showCheckBox) {
    bool isCheck = false;
  ...
  void saveOrUpdateData() {
    ...
    if (selectGuid.trim().isEmpty) {
      if (imageFile.path.isNotEmpty) {
        context.read<SaleChannelBloc>().add(SaleChannelWithImageSave(
              salechannel: screenData,
              imageFile: imageFile,
              imageWeb: imageWeb,
            ));
      } else {
        context.read<SaleChannelBloc>().add(SaleChannelSave(salechannelmodel: screenData));
      }
    } else {
  ...
  void updateData(String guid) {
    showCheckBox = false;
    if (imageWeb != null) {
      context.read<SaleChannelBloc>().add(SaleChannelWithImageUpdate(
            guid: guid,
            salechannel: screenData,
            imageFile: imageFile,
            imageWeb: imageWeb!,
      ...
      context.read<SaleChannelBloc>().add(SaleChannelUpdate(guid: guid, salechannelmodel: screenData));
    }
  }
    ...
    formWidgets.add(Padding(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            floatingLabelBehavior: FloatingLabelBehavior.always,
            border: const OutlineInputBorder(),
            labelText: global.language("sale_channel_name"),
          ),
        )));
    formWidgets.add(
      Padding(
          ...
          children: [
            Expanded(
              child: TextField(
                ...
                onChanged: (value) {
                  isDataChange = true;
                  screenData.gp = (value.isEmpty) ? 0 : double.tryParse(value)!;
                },
                onSubmitted: (value) {
                  if (kIsWeb) {
                    findFocusNext(focusNodeIndex);
                ...
                focusNode: fieldFocusNodes[++focusNodeMax].focusNode,
                textAlign: TextAlign.left,
                controller: TextEditingController(text: screenData.gp.toString()),
                decoration: InputDecoration(
                  floatingLabelBehavior: FloatingLabelBehavior.always,
                  border: const OutlineInputBorder(),
                ...
                children: [
                  Expanded(
                    ...
                    children: [
                      Radio(
                          value: 0,
                          groupValue: screenData.gptype,
                          onChanged: (value) {
                            if (isEditMode) {
                              setState(() {
                                screenData.gptype = value!;
                              });
                            }
                          }),
                      Expanded(
                          child: Text(
                        global.language("sale_channel_gptype_baht"),
                  ...
                  Expanded(
                    ...
                    Expanded(
                      child: Text(
                        global.language("sale_channel_gptype_percent"),
                        overflow: TextOverflow.clip,
                      ),
                    )
                  ])),
          ...
          children: [
            Expanded(
              child: priceList.isEmpty
                  ? Text("version POS : ไม่มีราคา deliver")
                  : DropdownButton<PriceModel>(
                      value: priceList.firstWhereOrNull((element) => element.keyNumber == screenData.price) 
                          ?? priceList.firstOrNull,
                      onChanged: (PriceModel? value) {
                        if (isEditMode && value != null) {
                          setState(() {
                            screenData.price = value.keyNumber;
                          });
                        }
                      },
                      items: priceList.map((PriceModel value) {
                        return DropdownMenuItem<PriceModel>(
                          value: value,
                          child: Text(value.names.isNotEmpty ? value.names[0].name ?? '' : ''),
                        );
                      }).toList(),
                    ),
            ),
    ...
    return Scaffold(
        ...
        appBar: AppBar(
            ...
            leading: mobileScreen
                ...
                : null,
            title: Text(headerEdit + global.language("sale_channel")),
            actions: <Widget>[
              if (selectGuid.isNotEmpty)
                Padding(
                    padding: const EdgeInsets.only(right: 20.0),
                          ...
                          builder: (BuildContext context) => AlertDialog(
                            ...
                            actions: <Widget>[
                              ...
                              ElevatedButton(
                                  ...
                                  onPressed: () {
                                    Navigator.pop(context);
                                    context.read<SaleChannelBloc>().add(SaleChannelDelete(guid: selectGuid));
                                  },
              ...
              listeners: [
                BlocListener<SaleChannelBloc, SaleChannelState>(
                  listener: (context, state) {
                    ...
                    if (state is SaleChannelLoadFailed) {
                      setState(() {
                        loadingData = false;
                      });
                    }
                    // Save
                    if (state is SaleChannelSaveSuccess) {
                      setState(() {
                        global.showSnackBar(
                            context,
                            const Icon(
                    ...
                    if (state is SaleChannelSaveFailed) {
                      setState(() {
                        global.showSnackBar(
                            context,
                            const Icon(
                    ...
                    // Update
                    if (state is SaleChannelUpdateSuccess) {
                      setState(() {
                        global.showSnackBar(
                            context,
                            const Icon(
                    ...
                    if (state is SaleChannelUpdateFailed) {
                      setState(() {
                        global.showSnackBar(
                            context,
                            const Icon(
                    ...
                    // Delete
                    if (state is SaleChannelDeleteSuccess) {
                      setState(() {
                        global.showSnackBar(
                            context,
                            const Icon(
                    ...
                    // Delete Many
                    if (state is SaleChannelDeleteManySuccess) {
                      setState(() {
                        global.showSnackBar(
                            context,
                            const Icon(
                    ...
                    // Get
                    if (state is SaleChannelGetSuccess) {
                      setState(() {
                        isDataChange = false;

                        screenData = state.salechannel;
...
// Add this extension method at the end of the file, outside any class

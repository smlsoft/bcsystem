...
class ColorScreenState extends State<ColorScreen> with SingleTickerProviderStateMixin {
  ...
  void loadDataList(String search) {
    context.read<ColorBloc>().add(ColorLoadList(offset: (colorListDatas.isEmpty) ? 0 : colorListDatas.length, limit: global.loadDataPerPage, search: search));
  }
  ...
  void getData(String guid) {
    headerEdit = global.language("display");
    isEditMode = false;
  ...
  Widget listScreen({bool mobileScreen = false}) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(global.language("colors")),
        leading: IconButton(
                      ...
                      builder: (BuildContext context) => AlertDialog(
                        ...
                        actions: <Widget>[
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () {
                                Navigator.pop(context);
                                context.read<ColorBloc>().add(ColorDeleteMany(guid: colorGuidListChecked));
                              },
                              child: const Text('ลบ')),
                        ],
                      ),
            ...
            if (kIsWeb) {
              if (event is RawKeyDownEvent) {
                if (event.logicalKey == LogicalKeyboardKey.arrowUp) {
                  isKeyDown = false;
                  int index = colorListDatas.indexOf(colorListDatas.firstWhere((element) => element.guidfixed == selectGuid));
                  if (index > 0) {
            ...
            children: [
              ...
              Container(
                  ...
                  child: Row(children: [
                    Expanded(
                        flex: 5,
                        child: Text(global.language("color_code"),
              ...
              Expanded(child: SingleChildScrollView(controller: listScrollController, child: Column(children: colorListDatas.map((value) => listObject(value, showCheckBox)).toList())))
            ],
          )),
    );
  }
              ...
              Expanded(
                  ...
                  child: Row(children: [
                    Container(
                      width: 20,
                      height: 20,
                      decoration: BoxDecoration(color: global.colorFromHex(value.colorselecthex), border: Border.all(color: Colors.black)),
                    ),
                    const SizedBox(width: 10),
                    Container(
                      width: 20,
                      height: 20,
                      decoration: BoxDecoration(color: global.colorFromHex(value.colorsystemhex), border: Border.all(color: Colors.black)),
                    )
                  ])),
  ...
  void saveOrUpdateData() {
    ...
    if (selectGuid.trim().isEmpty) {
      ...
      context.read<ColorBloc>().add(ColorSave(color: colorModel));
    } else {
  ...
  void updateData(String guid) {
    ...
    context.read<ColorBloc>().add(ColorUpdate(guid: guid, color: colorModel));
  }
  ...
  void getDataToEditScreen(ColorModel color) {
    ...
    publicColorSelected = global.colorFromHex(color.colorsystemhex.replaceAll("#", ""));
                          ...
                          builder: (BuildContext context) => AlertDialog(
                            ...
                            actions: <Widget>[
                              ...
                              ElevatedButton(
                                  ...
                                  onPressed: () {
                                    Navigator.pop(context);
                                    context.read<ColorBloc>().add(ColorDelete(guid: selectGuid));
                                  },
                                  child: const Text('ลบ')),
                            ],
                          ),
              ...
              if (isSaveAllow == false && selectGuid.trim().isNotEmpty)
                Padding(
                    ...
                    child: IconButton(
                      ...
                      onPressed: () {
                        showCheckBox = false;
                        switchToEdit(colorListDatas[colorListDatas.indexOf(colorListDatas.firstWhere((element) => element.guidfixed == selectGuid))]);
                      },
                      icon: const Icon(
                        Icons.edit,
                      ),
                ...
                child: Container(
                    ...
                    child: Column(children: [
                      Form(
                          child: TextFormField(
                              ...
                              decoration: InputDecoration(
                                ...
                                labelText: global.language("color_code"),
                              ))),
                        ...
                        Padding(
                          ...
                          child: TextFormField(
                              ...
                              decoration: InputDecoration(
                                ...
                                contentPadding: const EdgeInsets.only(left: 10, top: 0, bottom: 0, right: 10),
                                floatingLabelBehavior: FloatingLabelBehavior.always,
                                labelText: "${global.language("color_name")} (${languageList[i].name})",
                              )),
                        ),
                      Container(
                          width: double.infinity,
                      ...
                      Container(
                          ...
                          decoration: BoxDecoration(color: publicColorSelected, border: Border.all(color: Colors.grey), borderRadius: BorderRadius.circular(5)),
                          child: Column(children: [
                            Container(
                                padding: const EdgeInsets.all(10),
                                decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(5)),
                                width: double.infinity,
                                child: const Center(child: Text("จัดเข้ากลุ่มสีในระบบ THAI7"))),
                            const SizedBox(height: 10),
                            Wrap(
                              spacing: 4.0,
                              runSpacing: 4.0,
                                ...
                                for (int i = 0; i < global.publicColors.length; i++)
                                  ElevatedButton(
                                    style: ElevatedButton.styleFrom(
                                        ...
                                        backgroundColor: global.colorFromHex(global.publicColors[i].color),
                                        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(5))),
                                    onPressed: () {
                                      setState(() {
                                        publicColorSelectedCode = global.publicColors[i].code;
                                        publicColorSelectedHex = global.publicColors[i].color;
                                        publicColorSelected = global.colorFromHex(publicColorSelectedHex);
                                      });
                                    },
                                    child: SizedBox(
                                        width: 50,
                                          ...
                                          children: [
                                            Expanded(
                                                child: Container(
                                                    ...
                                                    child: (publicColorSelectedCode == global.publicColors[i].code)
                                                        ? InvertColors(
                                                            child: DecoratedIcon(
                                                            icon: Icon(Icons.check, size: 32, color: global.colorFromHex(global.publicColors[i].color)),
                      ...
                      if (isSaveAllow)
                        SizedBox(
                            ...
                            child: ElevatedButton.icon(
                                ...
                                label: const Text((kIsWeb) ? "บันทึก (F10)" : "บันทึก")))
                    ])))));
  }

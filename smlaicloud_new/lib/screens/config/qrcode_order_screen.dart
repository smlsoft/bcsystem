...
class QrcodeOrderScreenState extends State<QrcodeOrderScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> generateAndShowQrcodeDialog(String qrData, String tableNumber) async {
    String shopName = "ร้าน : ${global.getShopName()}";
              ...
              ElevatedButton.icon(
                onPressed: () async {
                  await Printing.sharePdf(
                    ...
                    filename: 'qr_code_order_${global.getShopName()}_$tableNumber.png',
                  );
                  // ignore: use_build_context_synchronously
                            ...
                            children: [
                              ...
                              pw.Padding(
                                ...
                                child: pw.Column(
                                  children: [
                                    pw.Text(
                                      "ร้าน : ${global.getShopName()}",
                                      style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold),
                                    ),
                                    pw.Text(
                                      "โต๊ะ : ${tableList[tableIndex].number}",
                                      style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold, color: (tableList[tableIndex].number != "0") ? PdfColors.black : PdfColors.white),
                                    ),
                                  ],
                                ),
                              ),
                  ...
                  for (int i = 0; i < state.tables.length; i++) {
                    tableList.add(
                      TableModel(
                        ...
                        orderendcode: base64.encode(
                          utf8.encode(
                            '{"shopid": "${global.getShopId()}", "table": "${state.tables[i].number}"}',
                          ),
                        ),
                      ),
                    );

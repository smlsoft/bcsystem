...
class DebtorGroupScreen extends StatefulWidget {
  const DebtorGroupScreen({Key? key}) : super(key: key);

  @override
  State<DebtorGroupScreen> createState() => DebtorGroupScreenState();
}

class DebtorGroupScreenState extends State<DebtorGroupScreen> with SingleTickerProviderStateMixin {
  ...
  List<DebtorGroupModel> listData = [];
  DebtorGroupModel dataTemp = DebtorGroupModel(guidfixed: '', groupcode: '');
  List<String> debtorGroupGuidListChecked = [];
  ...
  late DebtorGroupState blocDebtorGroupState;
  String headerEdit = "";
  ...
  void loadDataList(String search) {
    context.read<DebtorGroupBloc>().add(DebtorGroupLoadList(offset: (listData.isEmpty) ? 0 : listData.length, limit: global.loadDataPerPage, search: search));
  }
  ...
  void getData(String guid) {
    ...
    context.read<DebtorGroupBloc>().add(DebtorGroupGet(guid: guid));
  }
  ...
  Widget listScreen({bool mobileScreen = false}) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(global.language('debtor_group')),
        leading: IconButton(
        ...
        actions: <Widget>[
                ...
                onPressed: () {
                  discardData(callBack: () {
                    setState(() {
                      if (showCheckBox) {
                        showCheckBox = false;
                        debtorGroupGuidListChecked.clear();
                      } else {
                        showCheckBox = true;
                        global.showSnackBar(
                            context,
          ...
          if (debtorGroupGuidListChecked.isNotEmpty)
            Padding(
                padding: const EdgeInsets.only(right: 20.0),
                child: IconButton(
                  focusNode: FocusNode(skipTraversal: true),
                      ...
                      builder: (BuildContext context) => AlertDialog(
                        ...
                        actions: <Widget>[
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () {
                                Navigator.pop(context);
                                context.read<DebtorGroupBloc>().add(DebtorGroupDeleteMany(guid: debtorGroupGuidListChecked));
                              },
  ...
  void switchToEdit(DebtorGroupModel value) {
    setState(() {
  ...
  Widget listObject(int index, DebtorGroupModel value, bool showCheckBox) {
    bool isCheck = false;
    for (int i = 0; i < debtorGroupGuidListChecked.length; i++) {
      if (debtorGroupGuidListChecked[i] == value.guidfixed) {
        isCheck = true;
        break;
      }
    }
        ...
        onTap: () {
          if (showCheckBox == true) {
            setState(() {
              selectGuid = value.guidfixed;
              if (isCheck == true) {
                debtorGroupGuidListChecked.remove(value.guidfixed);
              } else {
                debtorGroupGuidListChecked.add(value.guidfixed);
              }
              global.showSnackBar(
                  context,
                  const Icon(
                  ...
                  "${global.language("chosen")} ${debtorGroupGuidListChecked.length} ${global.language("list")}",
                  Colors.blue);
            });
          } else {
            setState(() {
  ...
  bool verifyData(DebtorGroupModel value) {
  ...
  void saveOrUpdateData() {
    if (screenEvent == global.ScreenEventEnum.edit || screenEvent == global.ScreenEventEnum.add) {
      DebtorGroupModel debtorGroupModel = DebtorGroupModel(
        guidfixed: "",
        groupcode: fieldTextController[0].text,
        names: packLanguage(),
      );
      if (verifyData(debtorGroupModel)) {
        showCheckBox = false;
        if (selectGuid.trim().isEmpty) {
          context.read<DebtorGroupBloc>().add(DebtorGroupSave(debtorGroups: debtorGroupModel));
        } else {
          showCheckBox = false;
          context.read<DebtorGroupBloc>().add(DebtorGroupUpdate(guid: selectGuid, debtorGroups: debtorGroupModel));
        }
      }
    }
  }

  void getDataToEditScreen(DebtorGroupModel debtorGroupModel) {
    isChange = false;
    dataTemp = debtorGroupModel;
    selectGuid = debtorGroupModel.guidfixed;
    fieldTextController[0].text = debtorGroupModel.groupcode;
    for (int i = 0; i < languageList.length; i++) {
      fieldTextController[i + 1].text = "";
    }
    for (int i = 0; i < languageList.length; i++) {
      for (int j = 0; j < debtorGroupModel.names.length; j++) {
        if (languageList[i].code! == debtorGroupModel.names[j].code) {
          fieldTextController[i + 1].text = debtorGroupModel.names[j].name;
        }
      }
    }
  }
                          ...
                          builder: (BuildContext context) => AlertDialog(
                            ...
                            actions: <Widget>[
                              ...
                              ElevatedButton(
                                  ...
                                  onPressed: () {
                                    Navigator.pop(context);
                                    context.read<DebtorGroupBloc>().add(DebtorGroupDelete(guid: selectGuid));
                                  },
                                  child: Text(global.language('confirm'))),
                            ],
                          ),
                        ...
                        Padding(
                          ...
                          child: TextFormField(
                            ...
                            decoration: InputDecoration(
                              ...
                              border: const OutlineInputBorder(),
                              labelText: "${global.language("debtor_name")} (${languageList[i].name})",
                            ),
                          ),
                        ),
                      if (isSaveAllow)
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: LayoutBuilder(builder: (context, constraints) {
          return BlocListener<DebtorGroupBloc, DebtorGroupState>(
              listener: (context, state) {
                blocDebtorGroupState = state;
                // Load
                if (state is DebtorGroupLoadSuccess) {
                  setState(() {
                    if (state.debtorGroups.isNotEmpty) {
                      listData.addAll(state.debtorGroups);
                    }
                  });
                }
                // Save
                if (state is DebtorGroupSaveSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is DebtorGroupSaveFailed) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                // Update
                if (state is DebtorGroupUpdateSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                if (state is DebtorGroupUpdateFailed) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                // Delete
                if (state is DebtorGroupDeleteSuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                // Delete Many
                if (state is DebtorGroupDeleteManySuccess) {
                  setState(() {
                    global.showSnackBar(
                        context,
                        const Icon(
                ...
                // Get
                if (state is DebtorGroupGetSuccess) {
                  setState(() {

...
class _BodyState extends State<Body> {
  ...
  initState() {
    super.initState();
    context.read<OptionBloc>().add(ListOptionLoad(page: 0, perPage: _perPage, search: _search, nextPage: true));
    ...
    if (_scrollController.offset >= _scrollController.position.maxScrollExtent && !_scrollController.position.outOfRange) {
      ...
      if (optionBloc.state is OptionLoadSuccess) {
        ...
        if (optionLoadSuccess.page!.page < optionLoadSuccess.page!.totalPage) {
          optionBloc.add(ListOptionLoad(page: optionLoadSuccess.page!.page + 1, perPage: _perPage, search: _search, nextPage: false));
        }
      }
    }
  }

...
class LoginPasswordScreenState extends State<LoginPasswordScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocListener<LoginBloc, LoginState>(
      ...
      child: Scaffold(
        body: Stack(
          children: [
            // We'll use a simple colored background instead of CloudBackground
            // as that might be specific to the SMLAI flavor
            Container(
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  ...
                  colors: [
                    ...
                    global.theme.primaryColor.withOpacity(0.7),
                  ],
                ),
              ),
            ),
  ...
  Widget buildLoginForm() {
    Widget userLoginWidget = (global.userLoginData.token.isNotEmpty)
        ? Column(children: [
            ...
            Text(global.userLoginData.email.isNotEmpty ? global.userLoginData.email : global.userLoginData.name,
              ...
              child: Stack(
                children: [
                  ...
                  Column(
                    ...
                    children: [
                      if (global.userLoginData.token.isEmpty) _buildUsernamePasswordLogin(),
                      userLoginWidget,
  ...
  Widget imageLogo() {
    ...
    return Image.asset(
      "assets/icons/Logo-Marine-New-Color-สีขาว.png", // แสดงโลโก้ DoHome
      height: 100,
    );
  }

  Widget _buildUsernamePasswordLogin() {
    return Column(
      children: [
        // Username field
        TextField(
          ...
          decoration: InputDecoration(
            labelText: global.language('username'),
            ...
            child: _isLoggingIn
                ? Row(
                    ...
                    children: [
                      ...
                      SizedBox(width: 12),
                      Text(global.language("logging_in"),
                          style: TextStyle(
                            fontSize: 16,
                            fontWeight: FontWeight.bold,
                          )),
  ...
  void _login() {
    ...
    if (username.isEmpty) {
      global.showSnackBar(context, const Icon(Icons.error, color: Colors.white), global.language("please_enter_username"), Colors.red);
      return;
    }

    if (password.isEmpty) {
      global.showSnackBar(context, const Icon(Icons.error, color: Colors.white), global.language("please_enter_password"), Colors.red);
      return;
    }

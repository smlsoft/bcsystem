...
class PrinterRepository {
  Future<ApiResponse> getPrinterList({
    ...
    try {
      String query = "/restaurant/printer/list?offset=$offset&limit=$limit&q=$search";
  ...
  Future<ApiResponse> deletePrinterMany(List<String> guids) async {
    Dio client = Client().init();
    try {
      final response = await client.delete('/restaurant/printer', data: guids);
      try {
        return ApiResponse.fromMap(response.data);
      } catch (ex) {
        throw Exception(ex);
  ...
  Future<ApiResponse> updatePrinter(String guid, PrinterModel printerModel) async {
    Dio client = Client().init();
    final data = printerModel.toJson();
    try {
      final response = await client.put('/restaurant/printer/$guid', data: data);
      try {
        return ApiResponse.fromMap(response.data);
      } catch (ex) {
        throw Exception(ex);

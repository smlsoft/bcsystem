...
class InventoryRepository {
  Future<ApiResponse> getInventoryList({int page = 0, int perPage = 1, String search = ""}) async {
    ...
    try {
      final response = await client.get('/inventory?page=${page}&limit=${perPage}&q=${search}&sort=code:1');
      try {
  ...
  Future<ApiResponse> saveInventory(InventoryModel inventory) async {
    Dio client = Client().init();
  ...
  Future<ApiResponse> updateInventory(InventoryModel inventory) async {
    Dio client = Client().init();
    final data = inventory.toJson();
    // print(data);
    try {
      final response = await client.put('/inventory/${inventory.guidfixed}', data: data);
      try {
        // final rawData = json.decode(response.toString());

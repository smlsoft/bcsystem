...
class KitchenRepository {
  ...
  Future<ApiResponse> getKitchenList({
    ...
    try {
      String query = "$url/list?offset=$offset&limit=$limit&q=$search&group-number=$groupNumber";
  ...
  Future<ApiResponse> getKitchenProductsList() async {
    Dio client = Client().init();

    try {
      String query = "$url/products";
      final response = await client.get(query);
      try {
        final rawData = json.decode(response.toString());
        if (rawData['error'] != null) {
  ...
  Future<ApiResponse> updateKitchen(String guid, KitchenModel printerModel) async {
    Dio client = Client().init();
    final data = printerModel.toJson();
    try {

...
import 'package:smlaicloud/repositories/option_repository.dart';
...
class OptionBloc extends Bloc<OptionEvent, OptionState> {
  ...
  void _onOptionLoadSelect(ListOptionLoadSelect event, Emitter<OptionState> emit) async {
    emit(OptionLoadSelectInProgress());
    try {
      final _result = await _optionRepository.getOptionList();

      List<OptionModel> _option = (_result.data as List).map((option) => OptionModel.fromJson(option)).toList();
  ...
  void _onOptionLoad(ListOptionLoad event, Emitter<OptionState> emit) async {
    ...
    List<OptionModel> _previousOption = [];
    ...
    try {
      final _result = await _optionRepository.getOptionList(perPage: event.perPage, page: event.page, search: event.search);
  ...
  void _onGetOptionId(ListOptionLoadById event, Emitter<OptionState> emit) async {
    emit(OptionLoadByIdInProgress());
    try {
      final _result = await _optionRepository.getOptionId(event.id);

      if (_result.success) {
        OptionModel _option = OptionModel.fromJson(_result.data);
        // // print(_option);

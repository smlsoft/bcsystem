...
class ProductBarcodeBloc extends Bloc<ProductBarcodeEvent, ProductBarcodeState> {
  ...
  void onProductBarcodeWithImageSave(ProductBarcodeWithImageSave event, Emitter<ProductBarcodeState> emit) async {
    emit(ProductBarcodeSaveInProgress());
    try {
      ApiResponse result = await _productBarcodeRepository.uploadFileNoLimitSize(event.imageFile, event.imageWeb!);
      if (result.success) {
  ...
  void onProductBarcodeGetByBarcodeList(ProductBarcodeGetByBarcodeList event, Emitter<ProductBarcodeState> emit) async {
    emit(ProductBarcodeGetByBarcodeListInProgress());
    try {
      final result = await _productBarcodeRepository.getProductBarcodeByBarcode(event.barcodes);
      if (result.success) {
        List<ProductBarcodeModel> productbarcodes = (result.data as List).map((productbarcode) => ProductBarcodeModel.fromJson(productbarcode)).toList();
        emit(ProductBarcodeGetByBarcodeListSuccess(productBarcodes: productbarcodes));
      } else {
        emit(const ProductBarcodeGetByBarcodeListFailed(message: 'ProductBarcode Not Found'));
      }
    } catch (e) {
  ...
  void onProductBarcodeGetPriceHistory(ProductBarcodeGetPriceHistory event, Emitter<ProductBarcodeState> emit) async {
    emit(ProductBarcodeGetPriceHistoryInProgress());
    try {
      final result = await _productBarcodeRepository.getProductBarcodePriceHistory(
      ...
      if (result.success == true && result.data != null) {
        emit(ProductBarcodeGetPriceHistorySuccess(
          priceHistory: result.data!,
          pagination: result.pagination,
        ));
      } else {
        emit(const ProductBarcodeGetPriceHistoryFailed(message: 'Failed to load price history'));
      }
    } catch (e) {

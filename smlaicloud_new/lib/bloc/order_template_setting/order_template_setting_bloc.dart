...
import 'package:smlaicloud/repositories/order_template_setting_repository.dart';
...
class OrderTemplateSettingBloc extends Bloc<OrderTemplateSettingEvent, OrderTemplateSettingState> {
  ...
  OrderTemplateSettingBloc({required OrderTemplateSettingRepository orderTemplateSettingRepository})
    ...
    on<OrderTemplateSettingWithImageSave>(onOrderTemplateSettingWithImageSave);
    on<OrderTemplateSettingWithImageUpdate>(onOrderTemplateSettingWithImageUpdate);
  }
  ...
  void onOrderTemplateSettingLoad(OrderTemplateSettingLoadList event, Emitter<OrderTemplateSettingState> emit) async {
    ...
    try {
      ...
      if (results.success) {
        List<OrderTemplateSettingModel> orderTemplateSettings = (results.data as List).map((orderTemplateSettings) => OrderTemplateSettingModel.fromJson(orderTemplateSettings)).toList();
  ...
  void onOrderTemplateSettingWithImageSave(OrderTemplateSettingWithImageSave event, Emitter<OrderTemplateSettingState> emit) async {
    emit(OrderTemplateSettingSaveInProgress());
    try {
      ApiResponse result = await _orderTemplateSettingRepository.uploadImage(event.imageFile, event.imageWeb!);
      if (result.success) {
        UploadImageModel uploadImage = UploadImageModel.fromJson(result.data);
        OrderTemplateSettingModel orderTemplateSettingModel = event.orderTemplateSetting;
        orderTemplateSettingModel.lineoaimg = uploadImage.uri;
        await _orderTemplateSettingRepository.saveOrder(orderTemplateSettingModel);
        emit(OrderTemplateSettingSaveSuccess());
      } else {
        emit(OrderTemplateSettingSaveFailed(message: result.message));
      }
    } catch (e) {
      emit(OrderTemplateSettingSaveFailed(message: e.toString()));
    }
  }

  void onOrderTemplateSettingWithImageUpdate(OrderTemplateSettingWithImageUpdate event, Emitter<OrderTemplateSettingState> emit) async {
    emit(OrderTemplateSettingUpdateInProgress());
    try {
      ApiResponse result = await _orderTemplateSettingRepository.uploadImage(event.imageFile, event.imageWeb);
      if (result.success) {
        ...
        OrderTemplateSettingModel orderTemplateSettingModel = event.orderTemplateSetting;
        orderTemplateSettingModel.lineoaimg = uploadImage.uri;
        await _orderTemplateSettingRepository.updateOrder(event.guid, orderTemplateSettingModel);
        emit(OrderTemplateSettingUpdateSuccess());
      } else {
        emit(OrderTemplateSettingUpdateFailed(message: result.message));
      }
    } catch (e) {
      emit(OrderTemplateSettingUpdateFailed(message: e.toString()));
    }
  }
}

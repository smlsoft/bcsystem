...
class ProductBloc extends Bloc<ProductEvent, ProductState> {
  ...
  ProductBloc({
    ...
    on<ProductGetByBarcodes>(onGetProductByBarcodes);
  }
  ...
  void onGetProductByBarcodes(ProductGetByBarcodes event, Emitter<ProductState> emit) async {
    emit(ProductGetByBarcodesInProgress());
    try {
      final result = await _productBarcodeRepository.getProductBarcodeByBarcode(event.barcodes);

      if (result.success) {
        List<ProductBarcodeModel> products = (result.data as List).map((product) => ProductBarcodeModel.fromJson(product)).toList();
        emit(ProductGetByBarcodesSuccess(products: products, source: event.source));
      } else {
        emit(const ProductGetByBarcodesFailed(message: 'Products Not Found'));
      }
    } catch (e) {
      emit(ProductGetByBarcodesFailed(message: e.toString()));
    }
  }
}

...
Future<Uint8List> generateStockReceiveProduct(
...
pw.Widget _contentTable(pw.Context context, TransactionModel screenData) {
  const tableHeaders = ['ลำดับที่', 'บาร์โค้ด', 'รายการสินค้า', 'คลัง', 'ที่เก็บ', 'หน่วย', 'จำนวน', 'ต้นทุน', 'มูลค่า'];
  ...
  return pw.TableHelper.fromTextArray(
    ...
    data: _generateTableData(screenData, tableHeaders.length),
  );
}

List<List<String>> _generateTableData(TransactionModel screenData, int columnCount) {
  List<List<String>> data = [];
  for (int i = 0; i < screenData.details!.length; i++) {
    // Add main product row
    data.add(List<String>.generate(
      columnCount,
      (col) => _cellValue(i, col, screenData, isDescription: false),
    ));

    // Add description row if it exists
    if (screenData.details![i].description?.isNotEmpty ?? false) {
      data.add(List<String>.generate(
        columnCount,
        (col) => _cellValue(i, col, screenData, isDescription: true),
      ));
    }
  }
  return data;
...
String _cellValue(int rowIndex, int columnIndex, TransactionModel screenData, {bool isDescription = false}) {
  if (isDescription) {
    // For description row, only show description in the item name column (index 2)
    return columnIndex == 2 ? "หมายเหตุ: ${screenData.details![rowIndex].description}" : "";
  }

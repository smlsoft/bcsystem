...
class PromptPay {
  ...
  static String generateQRData(String target, {double? amount}) {
    ...
    var checksum = _getCrc16XOR().convert(utf8.encode(data.join())).toRadixString(16).toUpperCase();

    if (checksum.length < 4) {
      checksum = checksum.padLeft(4, '0');
    }
  ...
  static String getPromptPayQRWithNewAmount(String qrData, double amount) {
    ...
    var checksum = _getCrc16XOR().convert(utf8.encode(newQRData)).toRadixString(16).toUpperCase();

    if (checksum.length < 4) {
      checksum = checksum.padLeft(4, '0');
    }

    return newQRData + checksum;
  }
  ...
  static bool isQRDataValid(String qrData) {
    ...
    var newChecksum = _getCrc16XOR().convert(utf8.encode(qrDataWithOutChecksum)).toRadixString(16).toUpperCase();

    if (newChecksum.length < 4) {
      newChecksum = newChecksum.padLeft(4, '0');
    }

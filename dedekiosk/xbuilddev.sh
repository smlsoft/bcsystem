...
if [ $? -eq 0 ]; then
    ...
    mv build/app/outputs/flutter-apk/app-release.apk build/app/outputs/flutter-apk/dede-order-station-app-dev-$current_date_time.apk

    echo "APK renamed to dede-order-station-app-dev-$current_date_time.apk"
...
cd $remote_directory
mrm dede-order-station-app-dev-*.apk
quit
EOF
    ...
    lftp -u $ftp_username, $ftp_password $ftp_server <<EOF
cd $remote_directory
put build/app/outputs/flutter-apk/dede-order-station-app-dev-$current_date_time.apk
quit
EOF

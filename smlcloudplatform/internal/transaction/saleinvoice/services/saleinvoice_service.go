...
func (svc SaleInvoiceService) RecalPoint(shopID string, code string, authUsername string) error {
	...
	updateCust.Point = CalGetPoint - CalUsePoint
...
func (svc SaleInvoiceService) CreateSaleInvoice(shopID string, authUsername string, doc models.SaleInvoice) (string, string, error) {
	...
	if dataDoc.CustCode != "" {
		...
		calPoint := dataDoc.TotalAmount / 100
		dataDoc.GetPoint = calPoint

		findCust, err := svc.repoCust.FindByDocIndentityGuid(ctx, shopID, "code", dataDoc.CustCode)
		if err != nil {
			return "", "", err
		}
		if findCust.Code == "" {
			return "", "", fmt.Errorf("customer not found with code: %s", dataDoc.CustCode)
		}

		dataUpdate := custModels.DebtorDoc{}
		dataUpdate = findCust // ถ้า findCust เป็น pointer
		dataUpdate.GuidFixed = findCust.GuidFixed
		dataUpdate.UpdatedBy = authUsername
		dataUpdate.UpdatedAt = time.Now()
		dataUpdate.Point = findCust.Point + calPoint // ถ้า Point อาจเป็น zero-value อยู่แล้ว

		err = svc.repoCust.Update(ctx, shopID, dataUpdate.GuidFixed, dataUpdate)
		if err != nil {
			return "", "", err
		}
	}

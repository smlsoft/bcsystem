...
class NumberPad extends StatefulWidget {
  ...
  const NumberPad({super.key, required this.onChange, this.title, this.unitName, this.textAlign = TextAlign.right, this.backgroundColor = Colors.white, this.header = ""});
  ...
  Widget build(BuildContext context) {
    List<Widget> buttonList = [
      ...
      NumPadButton(
        ...
        callBack: () {
          Navigator.pop(context);
          String finalValue = numberStr;
          // ถ้าลงท้ายด้วยจุด ให้เติม 0 ต่อท้าย
          if (finalValue.endsWith('.')) {
            finalValue = finalValue + '0';
          }
          if (widget.unitName != null) {
            widget.onChange!(finalValue, widget.unitName);
          } else {
            widget.onChange!(finalValue);
          }
        },
      )
    ];
            ...
            child: Column(
              children: <Widget>[
                ...
                Container(
                    ...
                    child: Text(global.moneyFormat.format(double.tryParse(numberStr) ?? 0.0), textAlign: widget.textAlign, style: const TextStyle(fontSize: 32, fontWeight: FontWeight.bold))),
                Expanded(

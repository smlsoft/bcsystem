...
import 'package:dedecashier/edckbank/edc_field_element.dart';
...
class EDCResponse {
      ...
      if (fieldDatas != null) {
        ...
        while (index < fieldDatas.length) {
          ...
          if (isFieldOverLength == false) {
            // copy next 2 bytes
            List<int> fieldDataLengthBytes = fieldDatas
                .getRange(index + 2, index + 4)
                .toList();
            int dataLength = EDCResponse.calcHexBCDtoTextSize(
              fieldDataLengthBytes,
            );
            dataLength += 4;

            List<int> fieldElementBytes = fieldDatas
                .getRange(index, index + dataLength)
                .toList();
  ...
  void _extractFieldData(List<int> bytes) {
    ...
    AppLogger.debug('${element.fieldType} : ${element.data}');
  ...
  String _getResponseCode() {
    if (byteData != null) {
      List<int> targetBlock = byteData!
          .getRange(
            PRESENTATION_HEADER_RESPONSE_CODE_INDEX,
  ...
  String _getTransactionCode() {
    if (byteData != null) {
      List<int> targetBlock = byteData!
          .getRange(
            PRESENTATION_HEADER_TRANSACTION_CODE_INDEX,
  ...
  List<int>? _getFieldDataBytes() {
    if (byteData != null) {
      List<int> targetBlock = byteData!
          .getRange(FIELD_DATA_INDEX, byteData!.length - 1)
          .toList();
      return targetBlock;
    }
    return null;

...
import 'package:cocomerchant_lite/screen_search/kitchen_search_screen.dart';
...
class AddProductToKitchenScreen extends StatefulWidget {
  const AddProductToKitchenScreen({Key? key}) : super(key: key);
...
class AddProductToKitchenScreenState extends State<AddProductToKitchenScreen> with SingleTickerProviderStateMixin {
  ...
  final List<Color> chipColors = [
    const Color.fromARGB(255, 84, 248, 174),
    const Color.fromARGB(255, 194, 191, 246),
    const Color.fromARGB(255, 223, 109, 130),
              ...
              child: Align(
                ...
                child: Wrap(
                  ...
                  children: value.kitchens!.asMap().entries.map((entry) {
                    ...
                    final kitchens = entry.value;
                    final color = chipColors[index % chipColors.length];
                    return Chip(
                      backgroundColor: color,
                      label: Text('${kitchens.code} - ${global.packName(kitchens.names)}'),
                    );
                  }).toList(),
                ),
              ),
                      ...
                      onChanged: (value) {
                        ...
                        _debouncer.run(() {
                          setState(() {
                            ...
                            listData = listDataTeamp
                                .where((element) => element.barcode.contains(searchText.toUpperCase()) || global.packName(element.names).contains(searchText))
                                .toList();

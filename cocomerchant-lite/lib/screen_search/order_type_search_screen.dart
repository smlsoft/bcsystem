...
import 'package:cocomerchant_lite/bloc/order_type/order_type_bloc.dart';
...
class OrderTypeSearchScreen extends StatefulWidget {
  const OrderTypeSearchScreen({Key? key, required this.word}) : super(key: key);
  final String word;
...
class OrderTypeSearchScreenState extends State<OrderTypeSearchScreen> with SingleTickerProviderStateMixin {
  ...
  void loadDataList(String search) {
    context.read<OrderTypeBloc>().add(OrderTypeLoadList(offset: (orderTypetListData.isEmpty) ? 0 : orderTypetListData.length, limit: global.loadDataPerPage, search: search));
  }
            ...
            if (kIsWeb || Platform.isWindows || Platform.isLinux || Platform.isMacOS) {
              if (event is KeyDownEvent) {
                ...
                if (event.logicalKey == LogicalKeyboardKey.arrowUp) {
                  isKeyDown = false;
                  int index = orderTypetListData.indexOf(orderTypetListData.firstWhere((element) => element.guidfixed == selectGuid));
                  if (index > 0) {
          ...
          child: Column(
            children: [
              ...
              Container(
                  ...
                  child: Row(children: [
                    Expanded(flex: 5, child: Text(global.language("order_type_code"), style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold))),
                    Expanded(
                        flex: 10,
                        child: Text(
              ...
              Expanded(child: SingleChildScrollView(controller: listScrollController, child: Column(children: orderTypetListData.map((value) => listObject(value)).toList())))
            ],
          )),
    );
  }

  Widget listObject(OrderTypeModel value) {

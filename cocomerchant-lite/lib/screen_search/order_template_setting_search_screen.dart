...
class OrderSettingSearchScreenState extends State<OrderTemplateSettingSearchScreen> with SingleTickerProviderStateMixin {
  ...
  void loadDataList(String search) {
    context
        .read<OrderTemplateSettingBloc>()
        .add(OrderTemplateSettingLoadList(offset: (orderSettingListData.isEmpty) ? 0 : orderSettingListData.length, limit: global.loadDataPerPage, search: search));
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () {
            Navigator.pop(context, global.SearchGuidCodeNameModel(guid: "", code: "", names: [], isCancel: true));
          },
        ),
      ),
      body: Focus(
              ...
              if (event is KeyDownEvent) {
                ...
                if (event.logicalKey == LogicalKeyboardKey.tab) {
                  if (selectGuid != "") {
                    Navigator.pop(
                      context,
                      global.SearchGuidCodeNameModel(
  ...
  Widget listObject(OrderTemplateSettingModel value) {
    return GestureDetector(
        onTap: () {
          Navigator.pop(context, global.SearchGuidCodeNameModel(guid: value.guidfixed!, code: value.code!, names: [], isCancel: false));
        },
        child: Container(
            decoration: BoxDecoration(
